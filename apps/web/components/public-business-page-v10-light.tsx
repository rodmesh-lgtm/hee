...
import { BadgeCheck, BriefcaseBusiness, ChevronDown, Clock3, Globe2, Headphones, Images, Info, Mail, MapPin, MessageCircle, Phone, Share2, Sparkles, UserRound } from "lucide-react";
...
export function PublicBusinessPageV10Light({ business, publicUrl }: Props) {
  ...
  async function share() { if (navigator.share) { try { await navigator.share({ title: business.name, url: publicUrl }); return; } catch {} } try { await navigator.clipboard?.writeText(publicUrl); } catch {} }
  ...
  return <main dir="rtl" className="min-h-screen bg-[linear-gradient(180deg,#fbfaff_0%,#ffffff_26%,#fbfaff_100%)] text-[#17131b] selection:bg-violet-100">
    <div dir="ltr" className="pointer-events-none sticky top-0 z-50 mx-auto h-0 w-full max-w-[580px]"><a href="/" aria-label="HEE - الصفحة الرئيسية" className="pointer-events-auto absolute left-3 top-3 inline-flex h-10 items-center gap-1.5 rounded-full bg-white/95 px-2.5 shadow-[0_5px_20px_rgba(62,35,92,.08)] ring-1 ring-[#ece7f2] sm:left-4"><span className="text-[24px] font-black tracking-[-.08em] text-[#6f3bd2]">HEE</span><span className="h-1.5 w-1.5 rounded-full bg-[#9c6be8]" /></a><button onClick={() => void share()} aria-label="مشاركة الصفحة" className="pointer-events-auto absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full bg-white/95 text-[#6330bd] shadow-[0_5px_20px_rgba(62,35,92,.08)] ring-1 ring-[#ece7f2] transition active:scale-95 sm:right-4"><Share2 className="h-4 w-4" /></button></div>
...
function QuickAction({ href, onClick, icon: Icon, label, green }: { href?: string; onClick?: () => void; icon: any; label: string; green?: boolean }) { const c = "flex min-h-[58px] flex-col items-center justify-center rounded-[16px] border border-[#ece7f1] bg-white text-[10px] font-bold shadow-[0_6px_18px_rgba(55,35,70,.03)] transition active:scale-[.98]"; const body = <><Icon className={`mb-1 h-[17px] w-[17px] ${green ? "text-emerald-600" : "text-[#6f3bd2]"}`} />{label}</>; return href ? <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined} className={c}>{body}</a> : <button onClick={onClick} className={c}>{body}</button>; }
