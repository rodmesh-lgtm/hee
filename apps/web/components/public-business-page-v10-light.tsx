...
export function PublicBusinessPageV10Light({ business, publicUrl }: Props) {
  ...
  return <main dir="rtl" className="min-h-screen bg-[linear-gradient(180deg,#fbfaff_0%,#ffffff_26%,#fbfaff_100%)] text-[#17131b] selection:bg-violet-100">
    <div dir="ltr" className="pointer-events-none sticky top-0 z-50 mx-auto h-0 w-full max-w-[580px]"><Link href="/" aria-label="HEE - الصفحة الرئيسية" className="pointer-events-auto absolute left-3 top-3 inline-flex h-10 items-center gap-1.5 rounded-full bg-white/95 px-2.5 shadow-[0_5px_20px_rgba(62,35,92,.08)] ring-1 ring-[#ece7f2] sm:left-4"><span className="text-[24px] font-black tracking-[-.08em] text-[#6f3bd2]">HEE</span><span className="h-1.5 w-1.5 rounded-full bg-[#9c6be8]" /></Link><button onClick={() => void share()} aria-label="مشاركة الصفحة" className="pointer-events-auto absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full bg-white/95 text-[#6330bd] shadow-[0_5px_20px_rgba(62,35,92,.08)] ring-1 ring-[#ece7f2] transition active:scale-95 sm:right-4"><Share2 className="h-4 w-4" /></button></div>
    <div className="mx-auto w-full max-w-[580px] px-3 pb-6 pt-[62px] sm:px-4"><div className="text-center text-[10px] font-bold tracking-wide text-[#766d7b]">هوية أعمال رقمية</div>
      ...
      <footer className="mt-6 border-t border-[#eee9f2] py-5 text-center"><Link href="/" className="text-[25px] font-black tracking-[-.08em] text-[#6f3bd2]">HEE</Link><p className="mt-1 text-[10px] font-bold text-[#69606f]">هوية أعمال رقمية</p><p className="mt-2 text-[9px] text-[#9a929f]">hee.sa</p></footer>
    </div>
  </main>;
}
...
function ContactLine({ icon: Icon, label, value, href, green }: { icon: IconType; label: string; value: string; href: string; green?: boolean }) { return <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined} className="flex items-center gap-3 py-3"><Icon className={`h-4 w-4 shrink-0 ${green ? "text-emerald-600" : "text-[#6f3bd2]"}`} /><span className="min-w-0 flex-1"><b className="block text-[10px] text-[#756d79]">{label}</b><span className="mt-0.5 block truncate text-[12px] text-[#28232b]">{value}</span></span></a>; }
