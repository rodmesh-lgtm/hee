...
export function OnboardingClient() {
  ...
  const canCreate = useMemo(() => normalizeSlug(slug).length >= 4 && city.trim().length >= 2 && (whatsapp.trim().length >= 8 || phone.trim().length >= 8), [slug, city, whatsapp, phone]);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!canCreate || pending) { setError("اختر رابطًا صالحًا، وحدد مدينة المقر الرئيسي، وأضف وسيلة تواصل واحدة على الأقل."); return; }
    setError(""); setPending(true);
    try {
      const response = await fetch("/api/business/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name.trim(), slug: normalizeSlug(slug), businessType, entityType: businessType, businessCategory: businessType, description: description.trim(), shortDescription: description.trim(), city: city.trim(), whatsapp: whatsapp.trim(), phone: phone.trim(), primaryColor: "#00a99d", onboardingStep: "profile_created" }),
      });
  ...
  return <main dir="rtl" className="min-h-screen bg-[linear-gradient(180deg,#fbfaff_0%,#fff_45%,#f8f6ff_100%)] text-[#1f2552]">
    <div className="mx-auto w-full max-w-[680px] px-4 py-7 sm:py-10">
      ...
      <form onSubmit={onSubmit} className="mt-6 rounded-[28px] border border-[#e9e5f1] bg-white p-5 shadow-[0_24px_70px_-50px_rgba(73,48,125,.45)] sm:p-7" aria-label="إعداد صفحة المنشأة">
        {step === 0 ? <section className="space-y-4"><div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#e8faf7] text-[#008f87]"><Building2 className="h-5 w-5" /></span><h1 className="mt-4 text-2xl font-black">صفحتك تبدأ من هنا</h1><p className="mt-1 text-sm text-slate-500">اختر النشاط لنجهز الوحدات المناسبة تلقائيًا. يمكنك تغييره وترتيب صفحتك لاحقًا.</p></div><label className="block"><span className="mb-1.5 block text-sm font-black">اسم المنشأة</span><input value={name} onChange={(e) => setName(e.target.value)} autoFocus placeholder="مثال: شركة الرواد للمقاولات" className="h-12 w-full rounded-2xl border border-[#dce7e5] bg-[#fbfdfd] px-4 text-sm outline-none focus:border-[#00a99d]" /></label><label className="block"><span className="mb-1.5 block text-sm font-black">نشاط المنشأة</span><select value={businessType} onChange={(e) => setBusinessType(e.target.value)} className="h-12 w-full rounded-2xl border border-[#dce7e5] bg-[#fbfdfd] px-4 text-sm outline-none focus:border-[#00a99d]">{businessTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select><span className="mt-1.5 block text-[11px] leading-5 text-slate-500">يحدد الأقسام المقترحة وترتيبها فقط؛ لا يغيّر الهيدر أو شعار منشأتك.</span></label><label className="block"><span className="mb-1.5 block text-sm font-black">نبذة قصيرة</span><textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="ماذا تقدم منشأتك؟" className="min-h-[96px] w-full rounded-2xl border border-[#dce7e5] bg-[#fbfdfd] px-4 py-3 text-sm leading-6 outline-none focus:border-[#00a99d]" /></label></section> : null}
        {step === 1 ? <section className="space-y-4"><div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#f1edff] text-[#6543ce]"><Link2 className="h-5 w-5" /></span><h1 className="mt-4 text-2xl font-black">الرابط والتواصل</h1><p className="mt-1 text-sm text-slate-500">اختر رابط صفحتك، وحدد مدينة المقر الرئيسي، وأضف وسيلة تواصل واحدة على الأقل.</p></div><div className="rounded-2xl border border-[#e5e3ec] bg-[#faf9fd] px-4 py-3 text-sm font-bold text-slate-500" dir="ltr">ir.sa/<span className="text-[#6543ce]">{normalizeSlug(slug) || "your-business"}</span></div><label className="block"><span className="mb-1.5 block text-sm font-black">الرابط بالإنجليزية</span><input value={slug} onChange={(e) => setSlug(normalizeSlug(e.target.value))} dir="ltr" placeholder="alrowad" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] px-4 text-left text-sm outline-none focus:border-[#8b72dc]" /></label><div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-black">واتساب</span><div className="relative"><MessageCircle className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true" /><input value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} dir="ltr" inputMode="tel" placeholder="+9665xxxxxxxx" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] pr-11 pl-4 text-sm outline-none focus:border-[#8b72dc]" /></div></label><label><span className="mb-1.5 block text-sm font-black">الهاتف <small className="font-normal text-slate-400">اختياري</small></span><input value={phone} onChange={(e) => setPhone(e.target.value)} dir="ltr" inputMode="tel" placeholder="05xxxxxxxx" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] px-4 text-sm outline-none focus:border-[#8b72dc]" /></label></div><label className="block"><span className="mb-1.5 block text-sm font-black">مدينة المقر الرئيسي</span><div className="relative"><MapPin className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true" /><input value={city} onChange={(e) => setCity(e.target.value)} required minLength={2} maxLength={80} autoComplete="address-level2" placeholder="مثال: جدة" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] pr-11 pl-4 text-sm outline-none focus:border-[#8b72dc]" /></div></label></section> : null}
