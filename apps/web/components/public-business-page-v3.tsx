          ...
          <div className="p-3.5 sm:p-5 md:p-7">
            <div className="flex items-start gap-3 md:gap-5">
              <div className="flex h-[64px] w-[64px] shrink-0 items-center justify-center overflow-hidden rounded-[19px] border border-[#e7ece9] bg-[#f5f8f6] sm:h-[72px] sm:w-[72px] sm:rounded-[22px] md:h-[94px] md:w-[94px] md:rounded-[26px]">
                {business.logoUrl ? <Image src={business.logoUrl} alt={business.name} width={108} height={108} className="h-full w-full object-contain" unoptimized /> : <span className="text-2xl font-black text-[#0d7b5a]">{business.name.charAt(0)}</span>}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-start justify-between gap-2 sm:gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 items-start gap-1.5 sm:items-center sm:gap-2">
                      <h1 className="min-w-0 break-words text-[18px] font-black leading-[1.35] tracking-[-.02em] text-[#10211c] sm:text-[23px] md:text-[30px]">{compactName(business.name)}</h1>
                      {business.isVerified ? <button type="button" onClick={() => setVerificationOpen(true)} aria-label="عرض تفاصيل التوثيق" title="موثق لدى HEE" className="relative mt-0.5 inline-grid h-[22px] w-[22px] shrink-0 place-items-center rounded-[8px] bg-[#12a86f] text-white shadow-[0_0_0_3px_#e8f8f1] transition hover:scale-105 sm:mt-0 sm:h-[23px] sm:w-[23px]"><Check className="h-3.5 w-3.5 stroke-[3.4]" /></button> : null}
                    </div>
                    ...
                    <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[9px] font-semibold text-slate-500 sm:mt-2 sm:text-[10px] md:text-[11px]">
                      {[business.city, business.district].filter(Boolean).map((item) => <span key={item}>{item}</span>)}
                      {business.city || business.district ? <span className="h-1 w-1 rounded-full bg-slate-300" /> : null}
                      <span className="inline-flex items-center gap-1 text-emerald-700"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> صفحة أعمال نشطة</span>
                    </div>
                  </div>
                  <button type="button" onClick={() => setShareOpen((value) => !value)} className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 transition active:scale-95 hover:border-emerald-200 hover:text-emerald-700" aria-label="مشاركة الصفحة"><Share2 className="h-4 w-4" /></button>
                </div>
                {business.shortDescription ? <p className="mt-2.5 max-w-[760px] text-[10.5px] leading-[1.8] text-slate-600 sm:mt-3 sm:text-[11px] sm:leading-6 md:text-[13px] md:leading-7">{business.shortDescription}</p> : null}
              </div>
            </div>

            {availableActions.length > 0 ? <div className={`mt-4 grid gap-1.5 sm:mt-5 sm:gap-2 md:gap-3 ${actionGridClass}`}>
              {whatsapp ? <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noreferrer noopener" className={actionClass}><MessageCircle className="h-[18px] w-[18px] text-[#139b70] sm:h-5 sm:w-5" /> واتساب</a> : null}
              {phone ? <a href={`tel:${phone}`} className={actionClass}><Phone className="h-[18px] w-[18px] text-[#139b70] sm:h-5 sm:w-5" /> اتصال</a> : null}
              {mapHref ? <a href={mapHref} target="_blank" rel="noreferrer noopener" className={actionClass}><MapPin className="h-[18px] w-[18px] text-[#139b70] sm:h-5 sm:w-5" /> الموقع</a> : null}
              {departments.length > 0 ? <button type="button" onClick={() => document.getElementById("hee-contact-directory")?.scrollIntoView({ behavior: "smooth" })} className={actionClass}><Users className="h-[18px] w-[18px] text-[#139b70] sm:h-5 sm:w-5" /> التواصل</button> : null}
            </div> : null}

            {shareOpen ? <div className="mt-3.5 flex items-center gap-2.5 rounded-[16px] border border-[#e3ebe7] bg-[#f8faf9] p-2.5 sm:mt-4 sm:gap-3 sm:rounded-[18px] sm:p-3"><img src={qrDataUrl} alt="رمز QR" className="h-14 w-14 shrink-0 rounded-xl border border-white bg-white p-1 sm:h-16 sm:w-16" /><div className="min-w-0 flex-1"><div className="truncate text-[9px] font-bold text-slate-500 sm:text-[10px]">{publicUrl}</div><button onClick={copyLink} type="button" className="mt-1.5 inline-flex min-h-9 items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-[9px] font-black text-[#087653] shadow-sm active:scale-[.98] sm:mt-2 sm:text-[10px]"><Copy className="h-3.5 w-3.5" /> {copied ? "تم النسخ" : "نسخ رابط الصفحة"}</button></div></div> : null}
          </div>
        </section>
