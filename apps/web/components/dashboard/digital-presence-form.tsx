...
const input = "h-11 w-full rounded-xl border border-slate-200 bg-[#fbfdfd] px-3 text-sm text-slate-900 outline-none transition focus:border-[#00bfae] focus:bg-white focus:ring-4 focus:ring-[#00bfae]/10 aria-[invalid=true]:border-rose-400 aria-[invalid=true]:bg-rose-50";
...
export function DigitalPresenceForm({ business, presenceStatus }: Props) {
  ...
  return <section className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-[0_18px_50px_-42px_rgba(7,24,27,.45)] sm:p-5">
    <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e9fbf8] text-[#008f87]"><Globe2 className="h-4 w-4" /></span><div><h2 className="font-black text-slate-950">الحضور الرقمي وSEO</h2><p className="text-[9px] font-black tracking-[.12em] text-[#008f87]" dir="ltr">INFRO DIGITAL PRESENCE</p></div></div>
    <p className="mt-3 text-xs leading-6 text-slate-500">أكمل بيانات المنشأة التي تظهر للزوار ومحركات البحث ومنصات المشاركة. جميع الحقول اختيارية، والروابط الاجتماعية لا تُقبل إلا على نطاقاتها الرسمية.</p>
    ...
    <form action={updateDigitalPresenceAction} className="mt-5 space-y-5">
      <div><div className="mb-3 flex items-center gap-2 text-sm font-black text-slate-900"><Globe2 className="h-4 w-4 text-[#008f87]" />بيانات المنشأة</div><div className="grid gap-3 sm:grid-cols-2">
      ...
      <div><div className="mb-3 flex items-center gap-2 text-sm font-black text-slate-900"><Share2 className="h-4 w-4 text-[#008f87]" />الشبكات الاجتماعية</div><div className="grid gap-3 sm:grid-cols-2">
      ...
      <div><div className="mb-3 flex items-center gap-2 text-sm font-black text-slate-900"><Search className="h-4 w-4 text-[#008f87]" />ظهور محركات البحث</div><div className="grid gap-3">
        <label className="grid gap-1.5 text-xs font-bold text-slate-600"><span>عنوان SEO</span><input name="metaTitle" defaultValue={business.metaTitle ?? ""} maxLength={70} aria-invalid={invalid(presenceStatus, "metaTitle")} className={input} /><span className="font-normal text-slate-400">حتى 70 حرفًا. عند تركه فارغًا يستخدم INFRO اسم المنشأة.</span></label>
        <label className="grid gap-1.5 text-xs font-bold text-slate-600"><span>وصف SEO</span><textarea name="metaDescription" defaultValue={business.metaDescription ?? ""} maxLength={180} rows={3} aria-invalid={invalid(presenceStatus, "metaDescription")} className="w-full rounded-xl border border-slate-200 bg-[#fbfdfd] px-3 py-3 text-sm text-slate-900 outline-none transition focus:border-[#00bfae] focus:bg-white focus:ring-4 focus:ring-[#00bfae]/10 aria-[invalid=true]:border-rose-400 aria-[invalid=true]:bg-rose-50" /><span className="font-normal text-slate-400">حتى 180 حرفًا ويستخدم في نتائج البحث والمشاركة.</span></label>
      </div></div>
      <button type="submit" className="min-h-11 rounded-xl bg-[#07181b] px-5 text-sm font-black text-white transition hover:bg-[#0d2a2e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00bfae] focus-visible:ring-offset-2">حفظ الحضور الرقمي</button>
    </form>
  </section>;
}
