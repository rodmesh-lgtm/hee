...
export function PageSectionOrderEditor({initialOrder}:Props){
  ...
  async function persist(next:ManagedId[]){
    ...
    }catch(error){applyOrder(previous);setSaveState("error");setMessage(error instanceof Error?error.message:"تعذر حفظ الترتيب")}
  }
  ...
  return <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white" dir="rtl">
    <div className="relative bg-[#07181b] p-5 text-white sm:p-6"><div className="absolute -left-10 -top-14 h-40 w-40 rounded-full bg-[#00e5a8]/10 blur-3xl"/><div className="relative flex flex-wrap items-start justify-between gap-4"><div><span className="inline-flex items-center gap-2 text-[9px] font-black tracking-[.16em] text-[#5cebd7]" dir="ltr"><Sparkles className="h-4 w-4" aria-hidden="true"/>03 · COMPOSITION</span><h2 className="mt-2 text-xl font-black">رتّب هويتك كما تريد</h2><p className="mt-2 max-w-xl text-xs leading-6 text-slate-300">اسحب الأقسام بالماوس أو اللمس. وعلى الجوال ولوحة المفاتيح استخدم أزرار أعلى وأسفل. يُحفظ الترتيب الحقيقي فور اكتمال الحركة.</p></div><div aria-live="polite" aria-atomic="true" className="min-h-8 rounded-full border border-white/10 bg-white/[.06] px-3 py-2 text-xs font-black">{saveState==="saving"?<span className="inline-flex items-center gap-1.5 text-amber-200"><Loader2 className="h-3.5 w-3.5 motion-safe:animate-spin" aria-hidden="true"/>جارٍ الحفظ</span>:saveState==="saved"?<span className="inline-flex items-center gap-1.5 text-[#5cebd7]"><CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true"/>تم حفظ الترتيب</span>:saveState==="error"?<span className="inline-flex items-center gap-1.5 text-rose-300"><AlertTriangle className="h-3.5 w-3.5" aria-hidden="true"/>{message}</span>:<span className="text-slate-400">جاهز للترتيب</span>}</div></div></div>
    <div className="p-3 sm:p-5"><div className="grid gap-2" role="list" aria-label="ترتيب أقسام الصفحة">{order.map((id,index)=><div key={id} ref={node=>{if(node)itemRefs.current.set(id,node);else itemRefs.current.delete(id)}} role="listitem" data-section-id={id} className={`group grid grid-cols-[auto_1fr_auto] items-center gap-2 rounded-2xl border px-2.5 py-3 transition motion-reduce:transition-none sm:grid-cols-[auto_auto_1fr_auto] sm:gap-3 sm:px-3 ${dragging===id?"scale-[1.01] border-[#35e4cb] bg-[#effbf9] shadow-[0_16px_34px_-24px_rgba(0,143,135,.7)] motion-reduce:scale-100":"border-slate-100 bg-[#fbfdfd] hover:border-[#ccefe9]"}`}>
      <button type="button" aria-label={`اسحب لترتيب ${LABELS[id].title}`} className="grid h-11 w-11 touch-none place-items-center rounded-xl border border-slate-200 bg-white text-[#008f87] transition active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00bfae] focus-visible:ring-offset-2 motion-reduce:transition-none" onPointerDown={e=>{e.preventDefault();draggingRef.current=id;setDragging(id);e.currentTarget.setPointerCapture(e.pointerId)}} onPointerMove={e=>{if(draggingRef.current!==id)return;e.preventDefault();pointerMove(id,e.clientY)}} onPointerUp={e=>{if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId);stopDragging();void persist(orderRef.current)}} onPointerCancel={()=>{stopDragging();void persist(orderRef.current)}}><GripVertical className="h-5 w-5" aria-hidden="true"/></button>
      ...
      <div className="flex items-center gap-1"><button type="button" disabled={index===0||saveState==="saving"} onClick={()=>moveBy(id,-1)} aria-label={`نقل ${LABELS[id].title} للأعلى`} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:border-[#9fe8df] hover:text-[#008f87] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00bfae] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-30"><ArrowUp className="h-4 w-4" aria-hidden="true"/></button><button type="button" disabled={index===order.length-1||saveState==="saving"} onClick={()=>moveBy(id,1)} aria-label={`نقل ${LABELS[id].title} للأسفل`} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 transition hover:border-[#9fe8df] hover:text-[#008f87] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00bfae] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-30"><ArrowDown className="h-4 w-4" aria-hidden="true"/></button></div>
    </div>)}</div><div className="mt-4 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-[11px] leading-5 text-slate-500">السحب يعمل بالماوس واللمس، وأزرار النقل بديل دقيق على الشاشات الصغيرة وتقنيات المساعدة.</p><button type="button" disabled={saveState==="saving"||sameOrder(order,DEFAULT_ORDER)} onClick={()=>{applyOrder(DEFAULT_ORDER);void persist(DEFAULT_ORDER)}} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#ccefe9] bg-[#effbf9] px-3 text-xs font-black text-[#075d58] transition hover:bg-[#e4f8f4] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00bfae] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"><RotateCcw className="h-4 w-4" aria-hidden="true"/>الترتيب المقترح</button></div></div>
  </section>
}
