...
export function BottomActionBarEditor({ initialActions = [] }: Props) {
  ...
  const persist = async (next: typeof actions) => {
    if (JSON.stringify(next) === JSON.stringify(lastSaved.current)) return;
