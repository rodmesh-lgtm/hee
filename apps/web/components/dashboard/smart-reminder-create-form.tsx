...
export function SmartReminderCreateForm({
  ...
  return <form action={createSmartReminderAction} className="space-y-4" onSubmit={(event) => {
    ...
    {businessNoteId ? <input type="hidden" name="businessNoteId" value={businessNoteId} /> : null}
    <input type="hidden" name="timezone" defaultValue="" />
    {businessNoteId ? <div className="rounded-2xl border border-[#bdece6] bg-[#f2fcfa] p-4 text-xs font-bold leading-6 text-[#006f69]">سيتم ربط هذا التذكير بالمذكرة التي فتحت منها النموذج، ويمكنك العودة إليها من سياق العمل.</div> : null}
