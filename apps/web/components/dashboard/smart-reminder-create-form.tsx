...
export function SmartReminderCreateForm({
  ...
  return <form action={createSmartReminderAction} className="space-y-4" onSubmit={(event) => {
    ...
    <fieldset className="space-y-3"><legend className="text-sm font-black text-slate-800">اختر قنوات التنبيه</legend><p className="text-xs leading-6 text-slate-500">اختر قناة واحدة أو أكثر. لا تحتاج إلى ربط واتساب لاستخدام البريد أو إشعارات INFRO.</p><div className="grid gap-3 md:grid-cols-3"><label className={`flex items-start gap-3 rounded-2xl border p-4 ${whatsAppAvailable ? "cursor-pointer border-slate-200 bg-slate-50" : "cursor-not-allowed border-slate-200 bg-slate-100 opacity-60"}`}><input type="checkbox" name="deliveryChannels" value="whatsapp" defaultChecked={whatsAppAvailable} disabled={!whatsAppAvailable} className="mt-1 h-4 w-4 accent-[#009d93]" /><MessageCircleMore className="mt-0.5 h-5 w-5 shrink-0 text-[#009d93]" /><span><b className="text-xs text-slate-800">واتساب</b><small className="mt-1 block text-[11px] leading-5 text-slate-500">{whatsAppAvailable ? `إلى ${recipientLabel} عبر Meta والقالب المعتمد.` : "يتاح بعد ربط Meta وتوفر قالب تذكير معتمد."}</small></span></label><label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4"><input type="checkbox" name="deliveryChannels" value="email" className="mt-1 h-4 w-4 accent-[#009d93]" /><Mail className="mt-0.5 h-5 w-5 shrink-0 text-[#009d93]" /><span><b className="text-xs text-slate-800">البريد الإلكتروني</b><small className="mt-1 block text-[11px] leading-5 text-slate-500">إلى بريد الحساب {emailLabel}.</small></span></label><label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4"><input type="checkbox" name="deliveryChannels" value="in_app" defaultChecked={!whatsAppAvailable} className="mt-1 h-4 w-4 accent-[#009d93]" /><BellRing className="mt-0.5 h-5 w-5 shrink-0 text-[#009d93]" /><span><b className="text-xs text-slate-800">إشعار داخل INFRO</b><small className="mt-1 block text-[11px] leading-5 text-slate-500">يبقى محفوظًا في مركز الإشعارات حتى تقرأه.</small></span></label></div></fieldset>
    ...
    <div className="rounded-[24px] border border-[#bdece6] bg-[#f2fcfa] p-3 sm:p-4">
      ...
      <p className="mt-2 text-xs font-medium leading-6 text-[#426763]">زر التنفيذ — سيحفظ التذكير ويبدأ جدولتَه حسب الموعد والقنوات التي اخترتها.</p>
    </div>
  </form>;
}