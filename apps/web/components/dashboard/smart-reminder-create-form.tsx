...
export function SmartReminderCreateForm({ templateId, recipientLabel }: { templateId: string; recipientLabel: string }) {
  return <form
    action={createSmartReminderAction}
    className="space-y-4"
    onSubmit={(event) => {
  ...
  >
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <label className="space-y-2 text-sm font-bold text-slate-700">
        الموعد الأول
        <span className="relative block"><Clock3 className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400"/><input type="datetime-local" name="scheduledLocal" required className="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-4 pr-11 text-sm text-slate-900 outline-none transition focus:border-[#00aa9f] focus:ring-4 focus:ring-[#00aa9f]/10" /></span>
      </label>
      <label className="space-y-2 text-sm font-bold text-slate-700">
        التكرار
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <div className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4"><Clock3 className="mt-0.5 h-5 w-5 shrink-0 text-[#009d93]"/><div><p className="text-xs font-black text-slate-800">توقيت جهازك</p><p className="mt-1 text-xs leading-6 text-slate-500">يُحدد تلقائيًا من جهازك لحظة حفظ التذكير.</p><p className="mt-1 text-[10px] leading-5 text-slate-400">التكرار يحافظ على نفس الساعة المحلية حتى عند تغير التوقيت الصيفي.</p></div></div>
    </div>
