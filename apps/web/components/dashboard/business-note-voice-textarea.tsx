...
import { BrainCircuit, CalendarDays, Loader2, Mic, Square, UserRound, WandSparkles } from "lucide-react";
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => reject(new Error("MICROPHONE_REQUEST_TIMEOUT")), MICROPHONE_REQUEST_TIMEOUT_MS);
      }),
    ]);
  } finally {
...
export function BusinessNoteVoiceTextarea({ voiceAvailable }: { voiceAvailable: boolean }) {
  ...
  const fallbackTranscriptSeenRef = useRef(false);
  ...
  async function startRecording() {
    ...
    try {
      if (typeof MediaRecorder === "undefined") throw new Error("MICROPHONE_UNSUPPORTED");
      const stream = await microphoneStreamWithTimeout();
      ...
      recorder.onstop = () => {
        ...
        if (fallbackTranscriptSeenRef.current) {
          setMessage("تم تحويل الكلام إلى نص عبر المتصفح. راجع النص قبل الحفظ.");
          return;
        }
        setMessage("تم التقاط التسجيل، لكن خدمة التحويل إلى نص غير مفعّلة في هذه البيئة. الكتابة اليدوية ما زالت متاحة.");
      };
      recorder.start(750);
      ...
      if (name === "NotAllowedError" || name === "PermissionDeniedError") {
        setMessage("رفض المتصفح إذن الميكروفون لهذا الرابط. اسمح للميكروفون لهذا الموقع ثم أعد المحاولة.");
      } else if (name === "NotFoundError" || name === "DevicesNotFoundError") {
        setMessage("لم يعثر المتصفح على ميكروفون متاح.");
      } else if (name === "MICROPHONE_REQUEST_TIMEOUT") {
        setMessage("لم يرد المتصفح على طلب الميكروفون خلال 15 ثانية. أعد تحميل الصفحة وحاول مرة أخرى.");
      } else {
        setMessage("تعذر بدء التسجيل الصوتي. أعد المحاولة أو استخدم الكتابة اليدوية.");
      }
    } finally {
  ...
  return <div className="rounded-2xl border border-slate-200 bg-white">
    ...
    <div className="flex flex-col gap-3 border-t border-slate-100 bg-slate-50/70 p-3 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex flex-wrap items-center gap-2">
        {!recording ? <button type="button" onClick={startRecording} disabled={processing || organizing || requestingMicrophone} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-[#07181b] px-4 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-60">{requestingMicrophone ? <Loader2 className="h-4 w-4 animate-spin"/> : <Mic className="h-4 w-4"/>}{requestingMicrophone ? "جارٍ فتح الميكروفون..." : "تسجيل مذكرة صوتية"}</button> : <button type="button" onClick={stopRecording} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-rose-600 px-4 text-xs font-black text-white"><Square className="h-4 w-4"/>إيقاف {minutes}:{secs}</button>}
        ...
        <label className="flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-600"><span>لغة الكلام</span><select value={languageHint} onChange={(event) => setLanguageHint(event.target.value)} disabled={recording || processing || organizing || requestingMicrophone} className="bg-transparent font-bold outline-none">{LANGUAGE_OPTIONS.map(([option,label]) => <option key={option} value={option}>{label}</option>)}</select></label>
