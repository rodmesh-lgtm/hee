...
export function PublicTransactionLauncher({ slug, businessName, whatsapp, phone, bookingAvailable, hasWorkingHours, services }: Props) {
  ...
  return <>
    <div dir="rtl" className="fixed inset-x-0 bottom-0 z-[120] mx-auto flex w-full max-w-[580px] gap-2 border-t border-[#ece7f1] bg-white/95 p-3 shadow-[0_-14px_32px_rgba(55,35,70,.08)] backdrop-blur" style={{ paddingBottom: "max(12px, env(safe-area-inset-bottom))" }}>
      {canRequest ? <button onClick={() => setRequestOpen(true)} className="flex h-12 flex-1 items-center justify-center gap-2 rounded-2xl bg-[#6f3bd2] px-4 text-sm font-black text-white"><MessageCircle className="h-4 w-4" aria-hidden="true" />طلب خدمة</button> : null}
      {canBook ? <button ref={bookingOpenerRef} onClick={() => setBookingOpen(true)} className="flex h-12 flex-1 items-center justify-center gap-2 rounded-2xl border border-[#dcd5f4] bg-[#f7f4ff] px-4 text-sm font-black text-[#5d49cc]"><CalendarDays className="h-4 w-4" aria-hidden="true" />حجز موعد</button> : null}
    </div>

    {canRequest ? <PublicActionDialog open={requestOpen} onClose={() => setRequestOpen(false)} mode="request" businessName={businessName} whatsapp={whatsapp} phone={phone} title="طلب خدمة" description="سنسجل طلبك داخل iR ثم نجهز التواصل مع المنشأة." ctaLabel="تسجيل الطلب والمتابعة" /> : null}
    ...
    {bookingOpen ? <div dir="rtl" className="fixed inset-0 z-[260] flex items-start justify-center overflow-y-auto bg-black/70 p-3 sm:items-center" style={{ paddingTop: "max(12px, env(safe-area-inset-top))", paddingBottom: "max(12px, env(safe-area-inset-bottom))" }} onClick={closeBooking}>
      <div ref={bookingDialogRef} role="dialog" aria-modal="true" aria-labelledby="hee-booking-title" aria-describedby="hee-booking-description" className="my-auto flex max-h-[calc(100dvh-24px)] w-full max-w-[520px] flex-col overflow-hidden rounded-[24px] bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-[#eee9f2] p-4"><div><h2 id="hee-booking-title" className="text-lg font-black text-[#20264f]">حجز موعد</h2><p id="hee-booking-description" className="mt-1 text-xs text-slate-500">اختر الخدمة والموعد المناسب، وسيصل الحجز مباشرة للمنشأة.</p></div><button onClick={closeBooking} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-[#ece8f3] text-slate-500" aria-label="إغلاق"><X className="h-4 w-4" aria-hidden="true" /></button></div>
        {success ? <div className="overflow-y-auto p-5"><div role="status" aria-live="polite" className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold leading-7 text-emerald-700">{success}</div><button onClick={closeBooking} className="mt-4 h-11 w-full rounded-xl bg-[#6f3bd2] text-sm font-black text-white">إغلاق</button></div> : <form onSubmit={submitBooking} className="min-h-0 space-y-3 overflow-y-auto overscroll-contain p-4 sm:p-5">
          ...
          <button disabled={submitting} aria-busy={submitting} className="h-12 w-full rounded-2xl bg-[#6f3bd2] text-sm font-black text-white disabled:opacity-60">{submitting ? "جارٍ تسجيل الحجز..." : "تأكيد الحجز"}</button>
        </form>}
      </div>
    </div> : null}
  </>;
