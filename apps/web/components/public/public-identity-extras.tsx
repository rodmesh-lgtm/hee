  ...
  return <section aria-label="الهوية الرقمية للمنشأة" className="mx-auto mb-20 w-full max-w-[580px] space-y-2 px-3 sm:px-4" dir="rtl">
    {profileUrl ? <article id="company-profile-section" className="rounded-[18px] border border-[#e9e3ef] bg-white p-4 shadow-[0_8px_24px_rgba(55,35,70,.035)]">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link href={profileUrl} target="_blank" rel="noreferrer noopener" className="shrink-0 rounded-xl bg-[#6f3bd2] px-4 py-2.5 text-xs font-black text-white">عرض الملف</Link>
      </div>
    </article> : null}
    {socials.length ? <article className="rounded-[18px] border border-[#e9e3ef] bg-white p-4 shadow-[0_8px_24px_rgba(55,35,70,.035)]"><div className="flex items-center gap-2"><Globe2 className="h-4 w-4 text-[#6f3bd2]" /><h2 className="text-sm font-black text-[#302638]">حساباتنا الرسمية</h2></div><div className="mt-3 flex flex-wrap gap-2">{socials.map(([label, href]) => <Link key={label} href={href} target="_blank" rel="noreferrer noopener" className="rounded-xl border border-[#e7e1ef] bg-[#faf8fd] px-3 py-2 text-xs font-black text-[#5d49cc]">{label}</Link>)}</div></article> : null}
  </section>;
}
