...
const samples = [
  { name: "مطعم", detail: "القائمة · الفروع · الحجز", icon: UtensilsCrossed },
  { name: "عيادة", detail: "الخدمات · الفريق · الموقع", icon: Stethoscope },
  { name: "متجر", detail: "المنتجات · التواصل · الفروع", icon: Store },
  { name: "شركة خدمات", detail: "الخدمات · الفريق · التواصل", icon: BriefcaseBusiness },
];
...
export function HomepageProfessional() {
  ...
  return <main id="home" data-infro-home dir="rtl" className="min-h-screen overflow-x-clip bg-white text-slate-950">
    ...
    <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20"><div className="text-center"><p className="text-sm font-black text-[#008f87]">من الفكرة إلى مساحة عملك</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">ابدأ في 3 خطوات واضحة</h2></div><div className="mt-8 grid gap-4 md:grid-cols-3">{[["1","اختر اسم رابطك","تحقق من توفر اسم منشأتك ثم ابدأ التسجيل."],["2","ابنِ هويتك","أضف بيانات منشأتك وخدماتها وفروعها ووسائل التواصل."],["3","نظّم وتواصل","استخدم مذكرات الأعمال والتذكيرات وتسويق واتساب من مركز عمل واحد."]].map(([n,t,d]) => <div key={n} className="infro-home-card rounded-[24px] border border-[#dbe9e7] bg-white p-5 shadow-[0_16px_45px_-35px_rgba(0,95,88,.28)] sm:p-6"><span className="grid h-9 w-9 place-items-center rounded-full bg-[#008f87] text-sm font-black text-white">{n}</span><h3 className="mt-4 text-lg font-black sm:text-xl">{t}</h3><p className="mt-2 text-sm leading-7 text-slate-600 sm:text-base">{d}</p></div>)}</div></section>

    <section id="samples" className="bg-[#f5fffd] px-4 py-16 sm:px-6 sm:py-20"><div className="mx-auto max-w-6xl"><div className="text-center md:text-right"><p className="text-sm font-black text-[#008f87]">مرنة لكل نشاط</p><h2 className="mt-2 text-2xl font-black sm:text-3xl">صفحة تبدو كجزء من علامتك</h2><p className="mx-auto mt-3 max-w-2xl leading-7 text-slate-600 md:mx-0">من المطاعم والعيادات إلى المتاجر وشركات الخدمات، صُممت INFRO لتعرض أهم ما يحتاجه عميلك بسرعة ووضوح.</p></div><div className="-mx-4 mt-8 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-4 sm:mx-0 sm:grid sm:grid-cols-2 sm:overflow-visible sm:px-0 lg:grid-cols-4">{samples.map(({name,detail,icon:Icon}) => <div key={name} className="infro-home-card min-w-[78vw] snap-center rounded-[28px] border border-[#d7e9e7] bg-white p-5 shadow-sm sm:min-w-0"><div className="grid h-48 place-items-center rounded-[22px] border border-[#daf0ec] bg-[radial-gradient(circle_at_50%_20%,rgba(53,228,203,.22),transparent_45%),linear-gradient(145deg,#effbf9,#fff)]"><span className="grid h-20 w-20 place-items-center rounded-[26px] border border-white bg-white/90 text-[#008f87] shadow-[0_18px_40px_-25px_rgba(0,95,88,.42)]"><Icon className="h-9 w-9" /></span></div><h3 className="mt-5 text-lg font-black">{name}</h3><p className="mt-1 text-sm text-slate-500">{detail}</p></div>)}</div></div></section>
