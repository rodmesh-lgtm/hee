...
import { ArrowLeft, BadgeCheck, BarChart3, BriefcaseBusiness, Building2, Check, Eye, Link2, MapPin, MessageCircle, Palette, ShieldCheck, Sparkles, UsersRound } from "lucide-react";
...
export function HomepagePremium() {
  return <main dir="rtl" className="min-h-screen bg-[linear-gradient(180deg,#fbfaff_0%,#ffffff_38%,#f8f6ff_100%)] text-[#1f2552]">
    ...
    <section id="pricing" className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16"><div className="text-center"><span className="text-xs font-black text-[#6f3bd2]">باقات تنمو مع عملك</span><h2 className="mt-2 text-3xl font-black text-[#181d45]">ابدأ مجانًا ثم طوّر هويتك عند الحاجة.</h2><p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-[#716b7c]">ابدأ بالباقة المجانية، ثم اطلب الترقية من حسابك عندما تحتاج حدودًا ومزايا إضافية.</p></div><div className="mt-8 grid gap-4 lg:grid-cols-3">{plans.map((plan, index) => <article key={plan.name} className={`rounded-[26px] border bg-white p-5 ${index === 1 ? "border-[#bdaff1] shadow-[0_24px_60px_-46px_rgba(111,59,210,.7)]" : "border-[#ebe7f1]"}`}><div className="flex items-center justify-between"><h3 className="text-xl font-black text-[#24294f]">{plan.name}</h3><span className="rounded-full bg-[#f4f0ff] px-2.5 py-1 text-[10px] font-black text-[#6243c9]">{plan.badge}</span></div><p className="mt-3 min-h-12 text-sm leading-7 text-[#716b7c]">{plan.text}</p><div className="mt-4 space-y-2">{plan.items.map((item) => <div key={item} className="flex items-center gap-2 text-xs font-bold text-[#5f5969]"><Check className="h-4 w-4 text-[#6f3bd2]" />{item}</div>)}</div></article>)}</div></section>

    <section className="mx-auto max-w-6xl px-4 pb-14 sm:px-6 sm:pb-20"><div className="overflow-hidden rounded-[30px] bg-[#211b40] px-5 py-8 text-center text-white sm:px-8 sm:py-11"><Sparkles className="mx-auto h-6 w-6 text-[#baa6ff]" /><h2 className="mt-3 text-2xl font-black sm:text-3xl">اجعل أول انطباع عن منشأتك أكثر احترافية.</h2><p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-white/70">أنشئ هوية HEE الآن، عاينها، ثم شارك رابطك عندما تصبح جاهزًا.</p><Link href="/register" className="mt-6 inline-flex h-12 items-center gap-2 rounded-2xl bg-white px-6 text-sm font-black text-[#3f2b84]">أنشئ هويتك مجانًا<ArrowLeft className="h-4 w-4" /></Link></div></section>

    <footer className="border-t border-[#ece8f3] bg-white"><div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-7 text-center sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:text-right"><div><span className="text-xl font-black tracking-[-.08em] text-[#6f3bd2]">HEE</span><p className="mt-1 text-[11px] font-bold text-[#817a89]">هوية أعمال رقمية</p></div><div className="flex items-center justify-center gap-4 text-xs font-bold text-[#777183]"><Link href="/privacy">الخصوصية</Link><Link href="/terms">الشروط</Link><Link href="/login">تسجيل الدخول</Link></div></div></footer>
  </main>;
}
