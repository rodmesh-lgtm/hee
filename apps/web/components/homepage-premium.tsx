...
function BrandMark({ compact = false }: { compact?: boolean }) {
  return (
    <span className={`inline-flex items-end font-black leading-none tracking-[-0.12em] text-[#7146d9] ${compact ? "text-[22px]" : "text-[30px]"}`} aria-label="iR">
...
export function HomepagePremium() {
  return (
    <main dir="rtl" className="min-h-screen bg-[radial-gradient(circle_at_15%_0%,rgba(124,84,225,.12),transparent_34%),linear-gradient(180deg,#fbfaff_0%,#ffffff_40%,#f8f6ff_100%)] text-[#1f2552]">
      <header className="sticky top-0 z-50 border-b border-[#ece8f3]/80 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex min-h-[66px] max-w-6xl items-center justify-between px-4 sm:px-6">
          <Link href="/" className="inline-flex items-center gap-2" aria-label="iR"><BrandMark /></Link>
          <div className="flex items-center gap-2">
            <Link href="/login" className="inline-flex h-10 items-center rounded-xl px-3 text-xs font-black text-[#514b63] sm:px-4 sm:text-sm">دخول</Link>
            <Link href="/register" className="inline-flex h-10 items-center rounded-xl bg-[#7146d9] px-3 text-xs font-black text-white shadow-sm sm:px-4 sm:text-sm">أنشئ هويتك</Link>
          </div>
        </div>
      </header>
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-4 pb-14 pt-11 sm:px-6 sm:pt-16 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:gap-14 lg:pb-20">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full border border-[#ddd4fa] bg-[#f7f3ff] px-3 py-1.5 text-[11px] font-black text-[#6040c7]"><Sparkles className="h-3.5 w-3.5" />هوية أعمال رقمية على ir.sa</span>
          <h1 className="mt-5 max-w-3xl text-[2.2rem] font-black leading-[1.22] tracking-tight text-[#181d45] sm:text-5xl lg:text-[3.55rem]">كل أعمالك، بهوية واحدة تستحق أن تُشارك.</h1>
          <p className="mt-5 max-w-2xl text-[15px] leading-8 text-[#696477] sm:text-lg sm:leading-9">iR تمنح شركتك أو مؤسستك أو متجرك أو نشاطك الخدمي صفحة أعمال احترافية في رابط واحد؛ واضحة، موثوقة، وسهلة المشاركة مع عملائك.</p>
          <div className="mt-7 flex flex-col gap-3 sm:flex-row">
            <Link href="/register" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-[#7146d9] px-6 text-sm font-black text-white shadow-[0_18px_36px_-20px_rgba(113,70,217,.72)]">ابدأ مجانًا<ArrowLeft className="h-4 w-4" /></Link>
      ...
      <section className="border-y border-[#ece8f3] bg-white/65">
