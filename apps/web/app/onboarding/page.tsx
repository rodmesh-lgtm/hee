...
export default function OnboardingPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(result.error ?? "تعذر إنشاء الصفحة");
        return;
      }
      router.push(result.redirectTo ?? "/dashboard?welcome=1");
      router.refresh();
    } catch {
      setError("تعذر إنشاء الصفحة. حاول مرة أخرى.");
    } finally {
      setPending(false);
    }
  }
  ...
  return <main dir="rtl" className="min-h-screen bg-[linear-gradient(180deg,#fbfaff_0%,#fff_45%,#f8f6ff_100%)] text-[#1f2552]">
    <div className="mx-auto w-full max-w-[680px] px-4 py-7 sm:py-10">
      <div className="flex items-center justify-between"><div><div className="text-3xl font-black tracking-[-.08em] text-[#6f3bd2]">HEE</div><p className="mt-1 text-xs font-bold text-slate-500">أنشئ هويتك الرقمية</p></div><span className="rounded-full border border-[#e5e0f3] bg-white px-3 py-1.5 text-[11px] font-black text-[#6543ce]">{step + 1} / 2</span></div>
      <div className="mt-6 h-1.5 overflow-hidden rounded-full bg-[#ece8f6]"><div className="h-full rounded-full bg-[#6f3bd2] transition-all duration-300" style={{ width: step === 0 ? "50%" : "100%" }} /></div>

      <form onSubmit={onSubmit} className="mt-6 rounded-[28px] border border-[#e9e5f1] bg-white p-5 shadow-[0_24px_70px_-50px_rgba(73,48,125,.45)] sm:p-7">
        ...
        {step === 1 ? <section className="space-y-4"><div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#f1edff] text-[#6543ce]"><Link2 className="h-5 w-5" /></span><h2 className="mt-4 text-2xl font-black">الرابط والتواصل</h2><p className="mt-1 text-sm text-slate-500">اختر رابط صفحتك وأضف وسيلة تواصل واحدة على الأقل.</p></div><div className="rounded-2xl border border-[#e5e3ec] bg-[#faf9fd] px-4 py-3 text-sm font-bold text-slate-500" dir="ltr">hee.sa/<span className="text-[#6543ce]">{normalizeSlug(slug) || "your-business"}</span></div><label className="block"><span className="mb-1.5 block text-sm font-black">الرابط بالإنجليزية</span><input value={slug} onChange={(e) => setSlug(normalizeSlug(e.target.value))} dir="ltr" placeholder="alrowad" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] px-4 text-left text-sm outline-none focus:border-[#8b72dc]" /></label><div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-1.5 block text-sm font-black">واتساب</span><div className="relative"><MessageCircle className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={whatsapp} onChange={(e) => setWhatsapp(e.target.value)} dir="ltr" placeholder="+9665xxxxxxxx" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] pr-11 pl-4 text-sm outline-none focus:border-[#8b72dc]" /></div></label><label><span className="mb-1.5 block text-sm font-black">الهاتف <small className="font-normal text-slate-400">اختياري</small></span><input value={phone} onChange={(e) => setPhone(e.target.value)} dir="ltr" placeholder="05xxxxxxxx" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] px-4 text-sm outline-none focus:border-[#8b72dc]" /></label></div><label className="block"><span className="mb-1.5 block text-sm font-black">المدينة <small className="font-normal text-slate-400">اختياري</small></span><div className="relative"><MapPin className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={city} onChange={(e) => setCity(e.target.value)} placeholder="الرياض" className="h-12 w-full rounded-2xl border border-[#e5e3ec] bg-[#fbfbfd] pr-11 pl-4 text-sm outline-none focus:border-[#8b72dc]" /></div></label></section> : null}
