...
export async function rescheduleSmartReminderAction(form: FormData) { const context=await reminderContext(); const reminderId=field(form,"reminderId",128),timezone=field(form,"timezone",64),localDateTime=field(form,"scheduledLocal",32); if(!reminderId||!timezone||!localDateTime)redirect("/dashboard/reminders?reschedule=invalid"); try{const scheduledAt=reminderLocalDateTimeToUtc(localDateTime,timezone);await rescheduleSmartReminder({businessId:context.businessId,actorUserId:context.userId,reminderId,scheduledAt,timezone});revalidatePath("/dashboard/reminders");}catch(error){redirect(destinationFor(error,"reschedule"));} redirect("/dashboard/reminders?reschedule=success"); }
export async function pauseSmartReminderAction(form: FormData) { const context=await reminderContext(); const reminderId=field(form,"reminderId",128); if(!reminderId)redirect("/dashboard/reminders?pause=invalid"); try{await pauseSmartReminder({businessId:context.businessId,actorUserId:context.userId,reminderId});revalidatePath("/dashboard/reminders");}catch(error){redirect(destinationFor(error,"pause"));} redirect("/dashboard/reminders?pause=success"); }
export async function resumeSmartReminderAction(form: FormData) { const context=await reminderContext(); const reminderId=field(form,"reminderId",128); if(!reminderId)redirect("/dashboard/reminders?resume=invalid"); try{await resumeSmartReminder({businessId:context.businessId,actorUserId:context.userId,reminderId});revalidatePath("/dashboard/reminders");}catch(error){redirect(destinationFor(error,"resume"));} redirect("/dashboard/reminders?resume=success"); }
export async function cancelSmartReminderAction(form: FormData) { const context=await reminderContext(); const reminderId=field(form,"reminderId",128); if(!reminderId)redirect("/dashboard/reminders?cancel=invalid"); try{await cancelSmartReminder({businessId:context.businessId,actorUserId:context.userId,reminderId});revalidatePath("/dashboard/reminders");}catch(error){redirect(destinationFor(error,"cancel"));} redirect("/dashboard/reminders?cancel=success"); }
export async function completeSmartReminderAction(form: FormData) { const context=await reminderContext(); const reminderId=field(form,"reminderId",128); if(!reminderId)redirect("/dashboard/reminders?complete=invalid"); try{await completeSmartReminder({businessId:context.businessId,actorUserId:context.userId,reminderId});revalidatePath("/dashboard/reminders");}catch(error){redirect(destinationFor(error,"complete"));} redirect("/dashboard/reminders?complete=success"); }
export async function snoozeSmartReminderAction(form: FormData) { const context=await reminderContext(); const reminderId=field(form,"reminderId",128),minutes=Number(field(form,"minutes",8)); if(!reminderId||![10,30,60,1440].includes(minutes))redirect("/dashboard/reminders?snooze=invalid"); const rows=await db.$queryRaw<Array<{timezone:string}>>(Prisma.sql`SELECT "timezone" FROM "SmartReminder" WHERE "id"=${reminderId} AND "businessId"=${context.businessId} LIMIT 1`); if(!rows[0])redirect("/dashboard/reminders?snooze=failed"); try{await rescheduleSmartReminder({businessId:context.businessId,actorUserId:context.userId,reminderId,scheduledAt:new Date(Date.now()+minutes*60_000),timezone:rows[0].timezone});revalidatePath("/dashboard/reminders");}catch(error){redirect(destinationFor(error,"snooze"));} redirect("/dashboard/reminders?snooze=success"); }
