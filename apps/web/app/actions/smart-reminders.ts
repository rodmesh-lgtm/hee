...
export async function createSmartReminderAction(form: FormData) {
  ...
  const count = await db.$queryRaw<Array<{ count: bigint }>>(Prisma.sql`
    SELECT COUNT(*)::bigint AS "count" FROM "SmartReminder"
    WHERE "businessId"=${context.businessId} AND "status" IN ('scheduled','paused')
  `);
  if (Number(count[0]?.count ?? 0) >= MAX_ACTIVE_REMINDERS_PER_BUSINESS) redirect("/dashboard/reminders?create=limit");
  ...
  if (!title || !body || !timezone || !localDateTime) redirect("/dashboard/reminders?create=invalid");
  if (wantsWhatsApp && !recipientConsentAccepted) redirect("/dashboard/reminders?create=consent-required");
  if (wantsWhatsApp && !infroReminderWhatsAppReady()) redirect("/dashboard/reminders?create=whatsapp-unavailable");
...
export async function rescheduleSmartReminderAction(form: FormData) {
  ...
  redirect("/dashboard/reminders?reschedule=success&tab=upcoming");
}
...
export async function pauseSmartReminderAction(form: FormData) {
  ...
  redirect("/dashboard/reminders?pause=success&tab=paused");
}
...
export async function resumeSmartReminderAction(form: FormData) {
  ...
  redirect("/dashboard/reminders?resume=success&tab=upcoming");
}
...
export async function cancelSmartReminderAction(form: FormData) {
  ...
  redirect("/dashboard/reminders?cancel=success&tab=cancelled");
}
...
export async function completeSmartReminderAction(form: FormData) {
  ...
  redirect("/dashboard/reminders?complete=success&tab=completed");
}

export async function snoozeSmartReminderAction(form: FormData) {
  const context = await reminderContext(); const reminderId = field(form, "reminderId", 128), minutes = Number(field(form, "minutes", 8));
  ...
  redirect("/dashboard/reminders?snooze=success&tab=upcoming");
}
