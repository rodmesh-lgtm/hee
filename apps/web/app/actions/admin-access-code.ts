...
export async function createSubscriptionAccessCodeAdminAction(formData: FormData) {
 ...
 const plan=await db.businessPlan.findFirst({where:{code:planCode,isActive:true},select:{id:true}});
 if(!plan) redirect("/admin?access=invalid-plan");
 const plaintext=`HEE-${randomBytes(12).toString("hex").toUpperCase()}`;
 await db.subscriptionAccessCode.create({data:{codeHash:accessCodeHash(plaintext),label,planId:plan.id,createdByUserId:admin.id}});
 redirect(`/admin?access=created&newCode=${encodeURIComponent(plaintext)}`);
}
