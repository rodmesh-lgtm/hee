...
const GENERIC_RESET_MESSAGE = "إذا كان البريد مرتبطًا بحساب iR فستصلك رسالة الاستعادة خلال دقائق.";
...
export async function requestPasswordResetAction(_previous: PasswordResetState, formData: FormData): Promise<PasswordResetState> {
  ...
  if (!configured) return { error: "استعادة كلمة المرور عبر البريد لم تُفعّل بعد. تواصل مع إدارة iR." };
