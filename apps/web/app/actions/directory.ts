...
export async function deleteBranchAction(formData: FormData) {
  ...
  const result = await db.$transaction(async (tx) => {
    await lockDirectoryScope(tx, business.id, "branches");
    const retiring = await tx.branch.findFirst({ where: { id, businessId: business.id }, select: { id: true, isMain: true } });
    if (!retiring) return "missing" as const;
    await tx.branch.update({ where: { id: retiring.id }, data: { isActive: false, isMain: false } });
    if (retiring.isMain) {
      const replacement = await tx.branch.findFirst({ where: { businessId: business.id, isActive: true, id: { not: retiring.id } }, orderBy: [{ sortOrder: "asc" }, { createdAt: "asc" }], select: { id: true } });
      if (replacement) await tx.branch.update({ where: { id: replacement.id }, data: { isMain: true } });
    }
...
export async function createDepartmentAction(formData: FormData) {
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    const count = await tx.department.count({ where: { businessId: business.id, isActive: true } });
...
export async function deleteDepartmentAction(formData: FormData) {
  ...
  const result = await db.department.updateMany({ where: { id, businessId: business.id }, data: { isActive: false } });
...
export async function createContactPersonAction(formData: FormData) {
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    const count = await tx.contactPerson.count({ where: { businessId: business.id, isActive: true } });
    ...
    const isPrimary = count === 0 || formData.get("isPrimary") === "on";
...
export async function deleteContactPersonAction(formData: FormData) {
  ...
  const result = await db.$transaction(async (tx) => {
    await lockDirectoryScope(tx, business.id, "contacts");
    const retiring = await tx.contactPerson.findFirst({ where: { id, businessId: business.id }, select: { id: true, isPrimary: true } });
    if (!retiring) return "missing" as const;
    await tx.contactPerson.update({ where: { id: retiring.id }, data: { isActive: false, isPrimary: false } });
    if (retiring.isPrimary) {
      const replacement = await tx.contactPerson.findFirst({ where: { businessId: business.id, isActive: true, id: { not: retiring.id } }, orderBy: [{ sortOrder: "asc" }, { createdAt: "asc" }], select: { id: true } });
      if (replacement) await tx.contactPerson.update({ where: { id: replacement.id }, data: { isPrimary: true } });
    }
    return "retired" as const;
  });
