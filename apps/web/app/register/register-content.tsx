...
const oauthMessages: Record<string, string> = {
  "consent-required": "وافق على الشروط والأحكام وسياسة الخصوصية قبل إنشاء الحساب عبر Google.",
...
export function RegisterClient({ googleEnabled }: { googleEnabled: boolean }) {
  ...
  const googleHref = agreed ? "/api/auth/oauth/google?mode=register&consent=accepted" : "#social-consent";
  ...
  return <main dir="rtl" className="min-h-screen bg-[#f4f8f8] text-[#0a2426]">
    <div className="grid min-h-screen lg:grid-cols-[minmax(0,1fr)_minmax(480px,600px)]">
      ...
      <section className="flex items-center justify-center px-4 py-8 sm:px-8 lg:px-10"><div className="w-full max-w-md"><Link href="/" className="mb-6 block w-fit lg:hidden" aria-label="العودة إلى INFRO"><IrLogo className="h-12 w-auto" priority /></Link><div className="rounded-[28px] border border-[#dfe9e8] bg-white p-5 shadow-[0_28px_80px_-58px_rgba(7,24,27,.65)] sm:p-7">
        ...
        {googleEnabled ? <div className="mt-5" aria-label="إنشاء الحساب عبر مزود خارجي"><Link href={googleHref} prefetch={false} aria-disabled={!agreed} className={`flex h-12 items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-800 ${!agreed ? "opacity-50" : "hover:border-[#9fe8df] hover:bg-[#f7fcfb]"}`}><GoogleMark/><span>المتابعة باستخدام Google</span></Link></div> : null}
        ...
        {googleEnabled ? <div className="my-5 flex items-center gap-3" aria-hidden="true"><span className="h-px flex-1 bg-slate-200"/><span className="text-xs font-bold text-slate-400">أو بالبريد الإلكتروني</span><span className="h-px flex-1 bg-slate-200"/></div> : <div className="my-5" />}
