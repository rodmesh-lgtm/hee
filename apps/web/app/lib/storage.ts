...
class DatabaseStorageAdapter implements StorageAdapter {
  async upload(input: { file: File; folder: string }): Promise<StorageUploadResult> {
    ...
    const maxBytes = Number(process.env.COMPANY_PROFILE_MAX_BYTES ?? 5 * 1024 * 1024);
    if (file.size > maxBytes) {
      throw new Error("حجم الملف يجب أن يكون أقل من الحد المسموح");
    }
...
export function getPersistentStorageAdapter() {
  if (!persistentAdapter) {
    persistentAdapter = new DatabaseStorageAdapter();
  }
