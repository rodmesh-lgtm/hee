...
async function claimNext(database: PrismaClient, workerId: string, now: Date) {
  return database.$transaction(async (tx) => {
    ...
    await tx.$executeRaw(Prisma.sql`
      UPDATE "SmartReminderDelivery"
      SET "status"='processing', "leaseOwner"=${leaseOwner}, "leaseExpiresAt"=${new Date(now.getTime()+60_000)}, "attemptCount"="attemptCount"+1, "updatedAt"=${now}
...
async function loadContext(database: PrismaClient, delivery: ClaimedDelivery) {
  const rows = await database.$queryRaw<ReminderContext[]>(Prisma.sql`
    ...
    WHERE d."id"=${delivery.id} AND d."businessId"=${delivery.businessId} AND d."reminderId"=${delivery.reminderId}
      AND d."connectionId"=${delivery.connectionId} AND d."templateId"=${delivery.templateId}
    LIMIT 1
  `);
  return rows[0] ?? null;
}
...
async function acquireRateSlot(database: PrismaClient, delivery: ClaimedDelivery, limit: number, now: Date) {
  ...
  const rows = await database.$queryRaw<Array<{ sentCount: number }>>(Prisma.sql`
    INSERT INTO "WhatsAppSendRateBucket" ("connectionId","businessId","windowStart","sentCount","updatedAt") VALUES (${delivery.connectionId},${delivery.businessId},${windowStart},1,${now})
    ON CONFLICT ("connectionId","windowStart") DO UPDATE SET "sentCount"="WhatsAppSendRateBucket"."sentCount"+1,"updatedAt"=${now}
    WHERE "WhatsAppSendRateBucket"."businessId"=${delivery.businessId} AND "WhatsAppSendRateBucket"."sentCount"<${limit} RETURNING "sentCount"
  `);
...
export async function processNextSmartReminderDelivery(input: { database?: PrismaClient; workerId?: string; now?: Date; fetcher?: typeof fetch; env?: NodeJS.ProcessEnv; config?: MetaWhatsAppConfig } = {}) {
  ...
  if(!await hasActiveWhatsAppMarketingEntitlement({businessId:delivery.businessId,database,now})){
    await releaseAs(database,delivery,"cancelled",now,"WHATSAPP_MARKETING_ENTITLEMENT_REQUIRED");return {processed:true as const,result:"entitlement_required" as const,deliveryId:delivery.id};
  }
  ...
  if(!recipientStillOwnedByBusiness(context)){await releaseAs(database,delivery,"cancelled",now,"REMINDER_RECIPIENT_OWNERSHIP_CHANGED");return {processed:true as const,result:"recipient_changed" as const,deliveryId:delivery.id};}
  if(context.recipientConsentEvidence!==REMINDER_CONSENT_EVIDENCE||context.recipientConsentedAt.getTime()>now.getTime()){
    await releaseAs(database,delivery,"cancelled",now,"REMINDER_RECIPIENT_CONSENT_REQUIRED");return {processed:true as const,result:"consent_required" as const,deliveryId:delivery.id};
  }
  ...
  if(contact?.optedOutAt){await releaseAs(database,delivery,"cancelled",now,"REMINDER_RECIPIENT_OPTED_OUT");return {processed:true as const,result:"opted_out" as const,deliveryId:delivery.id};}
  if(context.connectionProvider!=="meta"||context.connectionStatus!=="connected"||context.templateProvider!=="meta"||context.templateStatus!=="approved"||!reminderTemplateSupportsBodyParameter(context.templateComponents)){
    await releaseAs(database,delivery,"cancelled",now,"REMINDER_OUTBOUND_CONFIGURATION_NOT_ACTIVE");return {processed:true as const,result:"cancelled" as const,deliveryId:delivery.id};
  }
  if(!await acquireRateSlot(database,delivery,outboundRateLimit(env),now)){
    const nextMinute=new Date((Math.floor(now.getTime()/60_000)+1)*60_000);await releaseAs(database,delivery,"retry_scheduled",now,"LOCAL_RATE_LIMIT",nextMinute);return {processed:true as const,result:"rate_limited" as const,deliveryId:delivery.id};
  }
  let config:MetaWhatsAppConfig,accessToken:string;
  try{config=input.config??getMetaWhatsAppConfig(env);accessToken=decryptWhatsAppCredential({envelope:credentialEnvelope(context.credentialEnvelope),encryptionKeyBase64:config.META_WHATSAPP_CREDENTIAL_ENCRYPTION_KEY,businessId:delivery.businessId});}
  catch{await releaseAs(database,delivery,"failed",now,"OUTBOUND_CONFIGURATION_INVALID");return {processed:true as const,result:"failed" as const,deliveryId:delivery.id};}
...
export async function runSmartReminderDeliveryWorker(input:Parameters<typeof processNextSmartReminderDelivery>[0]&{limit?:number}={}){const limit=Math.min(Math.max(input.limit??100,1),500);let processed=0;for(let index=0;index<limit;index+=1){const result=await processNextSmartReminderDelivery(input);if(!result.processed)break;processed+=1;}return {processed};}
