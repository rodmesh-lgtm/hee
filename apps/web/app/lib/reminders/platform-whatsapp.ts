...
export function getInfroReminderWhatsAppConfig(env: NodeJS.ProcessEnv = process.env): InfroReminderWhatsAppConfig | null {
  ...
  if (!/^v\d+\.\d+$/.test(graphVersion) || !/^\d{5,30}$/.test(phoneNumberId) || accessToken.length < 20 || !/^[a-z0-9_]{1,512}$/.test(templateName) || !/^[A-Za-z0-9_-]{2,20}$/.test(templateLanguage)) return null;
  ...
  return { graphVersion, phoneNumberId, accessToken, templateName, templateLanguage, ratePerMinute };
}
