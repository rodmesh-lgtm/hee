...
export function getInfroReminderWhatsAppPhoneNumberId(env: NodeJS.ProcessEnv = process.env) {
  const phoneNumberId = value(env, "INFRO_REMINDER_WHATSAPP_PHONE_NUMBER_ID");
  return /^\d{5,30}$/.test(phoneNumberId) ? phoneNumberId : null;
}
...
export function getInfroReminderWhatsAppConfig(env: NodeJS.ProcessEnv = process.env): InfroReminderWhatsAppConfig | null {
  ...
  const phoneNumberId = getInfroReminderWhatsAppPhoneNumberId(env);
  ...
  if (!/^v\d+\.\d+$/.test(graphVersion) || !/^\d{5,30}$/.test(wabaId) || !phoneNumberId || accessToken.length < 20 || !/^[a-z0-9_]{1,512}$/.test(templateName) || !/^[A-Za-z0-9_-]{2,20}$/.test(templateLanguage)) return null;
