...
export async function processNextWhatsAppReply(input: { database?: PrismaClient; workerId?: string; now?: Date; fetcher?: typeof fetch; env?: NodeJS.ProcessEnv; config?: MetaWhatsAppConfig } = {}) {
  ...
  await database.$transaction(async (tx) => { await tx.whatsAppMessage.create({ data: { id: randomUUID(), businessId: job.businessId, conversationId: context.conversation.id, provider: "meta", providerMessageId, direction: "outbound", messageType: "text", status: "sent", textBody: context.textBody, sentAt: now } }); await tx.whatsAppConversation.update({ where: { id_businessId: { id: context.conversation.id, businessId: job.businessId } }, data: { lastMessageAt: now, lastOutboundAt: now, slaRespondedAt: now } }); await tx.whatsAppReplyJob.update({ where: { id: job.id }, data: { status: "sent", providerMessageId, leaseOwner: null, leaseExpiresAt: null, lastErrorCode: null, lastErrorMessage: null } }); });
