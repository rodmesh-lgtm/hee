...
export async function processNextWhatsAppReply(input: { database?: PrismaClient; workerId?: string; now?: Date; fetcher?: typeof fetch; env?: NodeJS.ProcessEnv; config?: MetaWhatsAppConfig } = {}) {
  ...
  if (context.connection.provider !== "meta" || context.connection.status !== "connected") { await release(database, job.id, "failed", { lastErrorCode: "CONNECTION_NOT_READY" }); return { processed: true as const, result: "failed" as const }; }
