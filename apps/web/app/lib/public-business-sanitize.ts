...
export function sanitizePublicBusiness(input: unknown) {
  ...
  return {
    ...
    contactPersons: asRecords(business.contactPersons).map((contact) => {
      const department = asRecord(contact.department);
      return { id: String(contact.id ?? ""), name: clean(contact.name), jobTitle: clean(contact.jobTitle), imageUrl: clean(contact.imageUrl), phone: clean(contact.phone), whatsapp: clean(contact.whatsapp), isActive: contact.isActive !== false, department: department.name ? { name: String(department.name) } : null };
    }),
