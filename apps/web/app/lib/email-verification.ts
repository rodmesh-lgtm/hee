...
async function sendVerificationEmail(email: string, token: string) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    headers: {
      ...
      "User-Agent": "iR/1.0",
      "Idempotency-Key": `ir-email-verify-${hashToken(token).slice(0, 32)}`,
    },
    body: JSON.stringify({
      from,
      to: [email],
      subject: "تأكيد بريد حساب iR",
      text: `لتأكيد ملكيتك لبريد حساب iR افتح الرابط التالي خلال 24 ساعة:\n${verifyUrl}\n\nإذا لم تطلب إنشاء الحساب فتجاهل الرسالة.`,
      html: `<div dir="rtl" style="font-family:Arial,sans-serif;line-height:1.8"><h2>تأكيد بريد حساب iR</h2><p>افتح الرابط ثم اضغط زر التأكيد لإثبات ملكيتك لهذا البريد. الرابط صالح لمدة 24 ساعة.</p><p><a href="${verifyUrl}" style="display:inline-block;background:#6f3bd2;color:#fff;text-decoration:none;padding:12px 18px;border-radius:12px">تأكيد البريد الإلكتروني</a></p><p style="color:#666;font-size:13px">إذا لم تطلب إنشاء الحساب فتجاهل هذه الرسالة.</p></div>`,
    }),
    cache: "no-store",
