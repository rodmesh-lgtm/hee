...
const optionalUrlSchema = z
  ...
  .refine((value) => !value || /^https?:\/\//i.test(value), "الرابط يجب أن يبدأ بـ http:// أو https://");

export const registerSchema = z
  .object({
    name: z.string().min(2, "الاسم مطلوب"),
    email: z.string().email("البريد الإلكتروني غير صالح"),
    password: z.string().min(8, "كلمة المرور يجب أن تكون 8 أحرف على الأقل").refine((value) => passwordComplexityRegex.test(value), "يجب أن تحتوي كلمة المرور على حرف كبير وحرف صغير ورقم ورمز"),
...
export const productSchema = z.object({
  ...
  price: z
    ...
    .refine((value) => Number.isFinite(value) && value >= 0, "السعر يجب أن يكون رقمًا صالحًا"),
  oldPrice: z
    .string()
    .optional()
    .transform((value) => (value?.trim() ? Number(value.trim()) : null))
    .refine((value) => value === null || (Number.isFinite(value) && value >= 0), "السعر قبل الخصم يجب أن يكون رقمًا صالحًا"),
  imageUrl: z.string().url("رابط الصورة غير صالح").optional().or(z.literal("")),
  isActive: z.boolean().optional(),
});

export const businessProfileSchema = z.object({
  name: z.string().trim().min(2, "اسم النشاط بالعربية مطلوب"),
  nameEn: z
    .string()
    .trim()
    .min(2, "اسم النشاط بالإنجليزية مطلوب")
    .regex(/^[a-zA-Z0-9\s&'().,-]+$/, "اسم النشاط بالإنجليزية يجب أن يحتوي على أحرف إنجليزية صحيحة"),
  businessType: z.string().trim().min(2, "تصنيف النشاط مطلوب"),
  shortDescription: z.string().trim().min(6, "الوصف المختصر مطلوب").or(z.literal("")),
  description: z.string().trim().min(10, "وصف النشاط مطلوب"),
  logoUrl: z.string().trim().min(1, "يرجى رفع شعار النشاط"),
  coverUrl: z.string().trim().min(1, "يرجى رفع صورة الغلاف"),
  phone: z.string().trim().min(6, "رقم الهاتف مطلوب"),
  whatsapp: z.string().trim().min(6, "رقم واتساب مطلوب"),
