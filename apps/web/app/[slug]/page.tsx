...
function absolutePublicAssetUrl(value?: string | null) {
  ...
  if (!raw) return null;
  try { return new URL(raw, "https://hee.sa").toString(); } catch { return null; }
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: { absolute: title }, description, alternates: { canonical: canonicalUrl },
    openGraph: { title, description, type: "website", url: canonicalUrl, siteName: "HEE", locale: "ar_SA", images: socialImageUrl ? [{ url: socialImageUrl, alt: business.name }] : undefined },
    twitter: { card: socialImageUrl ? "summary_large_image" : "summary", title, description, images: socialImageUrl ? [socialImageUrl] : undefined },
...
export default async function PublicBusinessPageRoute({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const mapHref = normalizeHttpUrl(business.googleMapsLink) || (business.address ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([business.address, business.city].filter(Boolean).join(" "))}` : null);
  const mobileWhatsapp = String(business.whatsapp ?? "").replace(/\D/g, "");
  const mobilePhone = String(business.phone ?? "").trim();
  const hasContacts = business.departments.some((department) => department.isActive && department.contacts.some((contact) => contact.isActive && contact.name.trim()));
  const structuredData = {
    "@context": "https://schema.org", "@type": "LocalBusiness", "@id": `${canonicalUrl}#business`, name: business.name,
    ...(business.nameEn ? { alternateName: business.nameEn } : {}), url: canonicalUrl,
    ...(business.description ? { description: business.description } : {}), ...(logoUrl ? { logo: logoUrl } : {}), ...(imageUrl ? { image: imageUrl } : {}),
    ...(business.phone ? { telephone: business.phone } : {}), ...(business.email ? { email: business.email } : {}),
    ...(business.address || business.city || business.district ? { address: { "@type": "PostalAddress", ...(business.address ? { streetAddress: business.address } : {}), ...(business.district ? { addressLocality: business.district } : {}), ...(business.city ? { addressRegion: business.city } : {}), ...(business.country ? { addressCountry: business.country } : { addressCountry: "SA" }) } } : {}),
  ...
  return (
    <>
      ...
      <PublicV3MobileDock whatsapp={mobileWhatsapp || null} phone={mobilePhone || null} mapHref={mapHref} hasContacts={hasContacts} hasAbout={Boolean(business.description)} />
    </>
  );
}
