...
export default async function PublicBusinessPageRoute({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: safeJsonLd(structuredData) }} /><PublicBusinessAnalytics slug={business.slug} /><div className="pb-20"><PublicBusinessPageV10Light business={publicBusiness} qrDataUrl={qrDataUrl} publicUrl={publicUrl} /></div><PublicTransactionLauncher slug={publicBusiness.slug} businessName={publicBusiness.name} whatsapp={publicBusiness.whatsapp} phone={publicBusiness.phone} bookingAvailable={publicBusiness.bookingAvailable} services={publicBusiness.services.map((service) => ({ id: service.id, name: service.name, bookingEnabled: service.bookingEnabled, durationMinutes: service.durationMinutes }))} /></>;
}
