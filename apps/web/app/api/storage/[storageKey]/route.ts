...
const publicOwnerWhere = { deletedAt: null, emailVerifiedAt: { not: null } } as const;
...
async function publicImageReferenceBusiness(storageKey: string) {
  ...
  return db.business.findFirst({
    where: {
      isPublished: true,
      deletedAt: null,
      owner: publicOwnerWhere,
      OR: [
        { logoUrl: url }, { coverUrl: url },
...
async function publicCompanyProfileBusiness(storageKey: string, tenantId: string | null) {
  const url = `/api/storage/${storageKey}`;
  return db.business.findFirst({
    where: { isPublished: true, deletedAt: null, owner: publicOwnerWhere, companyProfileUrl: url, ...(tenantId ? { id: tenantId } : {}) },
    select: { id: true },
  });
}
...
async function loadAuthorizedBytes(storageKey: string) {
  ...
  catch (error) { console.error("[storage] failed to read persistent object", { storageKey, error }); return null; }
}
...
export async function GET(_request: Request, { params }: { params: Promise<{ storageKey: string }> }) {
  ...
  const metadata = await db.storedObject.findUnique({ where: { id: storageKey }, select: { id: true, folder: true, fileName: true, mimeType: true, size: true } });
  ...
  if ((metadata.folder === "company-profiles" || metadata.folder.startsWith("company-profiles/")) && metadata.mimeType === "application/pdf") {
    if (!(await publicCompanyProfileBusiness(metadata.id, tenantId))) return NextResponse.json({ error: "الملف غير متاح" }, { status: 404 });
    const stored = await loadAuthorizedBytes(storageKey); if (!stored) return NextResponse.json({ error: "تعذر قراءة الملف" }, { status: 503 });
    const safeName = metadata.fileName.replace(/[\r\n"\\]/g, "-") || "company-profile.pdf";
    return new NextResponse(new Uint8Array(stored.data), { status: 200, headers: {
      "Content-Type": "application/pdf", "Content-Length": String(metadata.size), "Content-Disposition": `inline; filename="${safeName}"`,
      "Cache-Control": AUTHORIZED_FILE_CACHE_CONTROL, "X-Content-Type-Options": "nosniff", "Content-Security-Policy": "default-src 'none'; frame-ancestors 'self'; sandbox",
    } });
  }
  ...
  if (!referencingBusiness || (tenantId && referencingBusiness.id !== tenantId)) return NextResponse.json({ error: "الملف غير متاح" }, { status: 404 });
  const stored = await loadAuthorizedBytes(storageKey); if (!stored) return NextResponse.json({ error: "تعذر قراءة الملف" }, { status: 503 });
  return new NextResponse(new Uint8Array(stored.data), { status: 200, headers: {
    "Content-Type": metadata.mimeType, "Content-Length": String(metadata.size), "Content-Disposition": "inline", "Cache-Control": AUTHORIZED_FILE_CACHE_CONTROL,
    "X-Content-Type-Options": "nosniff", "Content-Security-Policy": "default-src 'none'; img-src 'self'; sandbox",
  } });
}
