...
const orderSchema = z.object({
  ...
  items: z.array(z.object({ productId: z.string().trim().min(1).max(120), quantity: z.number().int().positive().max(100) })).min(1).max(50),
});

function rateLimited(retryAfterSeconds: number) {
  return NextResponse.json({ error: "تم إرسال عدد كبير من الطلبات خلال فترة قصيرة. حاول مرة أخرى لاحقاً." }, { status: 429, headers: { "Retry-After": String(Math.max(1, retryAfterSeconds)) } });
}
...
export async function POST(request: Request) {
  ...
  if (!business.acceptOnlineOrders) return NextResponse.json({ error: "استقبال الطلبات غير مفعل حالياً" }, { status: 403 });
  ...
  for (const item of parsed.data.items) {
    ...
    if (nextQuantity > 100) return NextResponse.json({ error: "كمية أحد المنتجات تتجاوز الحد المسموح" }, { status: 400 });
    aggregated.set(item.productId, nextQuantity);
  }

  const productIds = [...aggregated.keys()];
  const products = await db.product.findMany({ where: { businessId: business.id, isActive: true, deletedAt: null, id: { in: productIds } } });
  if (products.length !== productIds.length) return NextResponse.json({ error: "بعض المنتجات غير متاحة الآن" }, { status: 400 });

  const productsById = new Map(products.map((product) => [product.id, product]));
  const normalizedItems = productIds.map((productId) => {
    const product = productsById.get(productId)!;
    const quantity = aggregated.get(productId)!;
    return { productId: product.id, name: product.name, unitPrice: product.price, quantity, total: product.price * quantity };
  });

  const total = normalizedItems.reduce((sum, item) => sum + item.total, 0);
  if (!Number.isSafeInteger(total) || total < 0) return NextResponse.json({ error: "تعذر احتساب إجمالي الطلب" }, { status: 400 });
  ...
  const order = await db.$transaction(async (tx) => {
    ...
    return tx.order.create({ data: { businessId: business.id, customerId: customer.id, notes: parsed.data.notes || null, orderType: parsed.data.orderType, total, status: "pending", items: { create: normalizedItems } } });
  });
