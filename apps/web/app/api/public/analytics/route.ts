...
export async function POST(request: Request) {
  ...
  const payload = body && typeof body === "object" && !Array.isArray(body) ? body as { slug?: unknown; eventType?: unknown } : {};
  ...
  try {
    ...
    if (!rate.allowed) {
      return NextResponse.json({ ok: false }, { status: 429, headers: { "Retry-After": String(Math.max(1, rate.retryAfterSeconds)) } });
    }
