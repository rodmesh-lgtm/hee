...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string }> }) {
  ...
  const [events, plans, userCount, businessCount, publishedCount, verifiedCount, businesses] = await Promise.all([
  ...
  const filteredCount = await db.business.count({ where: businessWhere });
  ...
  return <main dir="rtl" className="min-h-screen bg-[#f7f8fb] px-4 py-8 text-[#1f2552] sm:px-6">
    <div className="mx-auto max-w-7xl space-y-5">
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Metric title="طلبات معلقة" value={pending.length} />
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-[24px] border border-[#e7e4f0] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><BriefcaseBusiness className="h-5 w-5 text-[#6f3bd2]" /><h2 className="font-black">طلبات الترقية</h2></div><div className="mt-4 space-y-2">{upgrades.length ? upgrades.map((event) => { const requestedPlan = String(meta(event.metadata).requestedPlan ?? "BUSINESS").toUpperCase(); const planReady = activePlans.has(requestedPlan); return <article key={event.id} className="flex flex-col gap-3 rounded-2xl border border-[#ece9f3] p-4 sm:flex-row sm:items-center sm:justify-between"><div><b className="text-sm">{event.business.name}</b><span className="mt-1 block text-xs text-slate-500">{event.business.plan?.code ?? "FREE"} ← {requestedPlan}</span>{!planReady ? <span className="mt-1 block text-[11px] font-bold text-rose-600">الباقة {requestedPlan} غير مهيأة.</span> : null}</div><div className="flex items-center gap-2"><Link href={`/admin/businesses/${event.business.id}`} className="rounded-xl border border-[#e5e1ec] px-3 py-2 text-xs font-black">مراجعة</Link><form action={approvePlanUpgradeAdminAction}><input type="hidden" name="eventId" value={event.id} /><button disabled={!planReady} className="h-9 rounded-xl bg-[#6f3bd2] px-4 text-xs font-black text-white disabled:bg-slate-300">اعتماد</button></form></div></article>; }) : <Empty text="لا توجد طلبات ترقية معلقة." />}</div></div>
      </section>
