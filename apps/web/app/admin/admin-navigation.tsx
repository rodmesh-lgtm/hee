...
function Item({ href, label, icon: Icon, badge, compact = false }: { href: string; label: string; icon: LucideIcon; badge?: string; compact?: boolean }) {
...
export function AdminMobileNavigation() {
  return <nav aria-label="تنقل الإدارة على الجوال" className="overflow-x-auto border-b border-slate-200 bg-white px-3 py-2 lg:hidden"><div className="flex min-w-max gap-1">{groups.flatMap((group) => group.items).map((item) => <Item key={item.href} {...item} compact />)}</div></nav>;
}
