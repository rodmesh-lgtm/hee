...
export function BookingFormsEditor({ initial }: { initial: BookingFormCatalog }) {
  ...
  return <form action={action} className="space-y-4">
    ...
    <fieldset disabled={pending} className="grid min-w-0 gap-5 lg:grid-cols-[1fr_320px]">
      ...
      <aside className={`${card} h-fit space-y-4 lg:sticky lg:top-24`}><p className="text-xs text-teal-600">معاينة مباشرة</p><h2 className="text-xl font-black">{form.title}</h2><p className="text-sm leading-7 text-slate-500">{form.description}</p><div className="rounded-xl border border-dashed border-slate-300 p-3 text-sm">رقم الجوال</div><div className="rounded-xl bg-teal-500/10 p-4 text-sm">اختر اليوم والفترة المناسبة</div>{form.fields.map(field => <div key={field.id} className="rounded-xl border border-dashed border-slate-300 p-3 text-sm">{field.label}{field.required ? " *" : " (اختياري)"}</div>)}{form.notesEnabled ? <div className="rounded-xl border border-dashed border-slate-300 p-3 text-sm">{form.notesLabel}</div> : null}<label className="grid gap-2 text-sm">النموذج المختار للنشر<select className={input} value={catalog.activeId} onChange={e => setCatalog({ ...catalog, activeId: e.target.value })}>{catalog.forms.map(item => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></aside>
    </fieldset>
