...
import { addSimpleServiceAction, updateSimpleServiceAction } from "../../actions/services";
...
export default async function DashboardServicesPage() {
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="space-y-2">{business.services.length ? business.services.map((service) => <form key={service.id} action={updateSimpleServiceAction} className="grid gap-2 rounded-[20px] border border-[#e7e9f4] bg-white p-3.5 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)_auto]"><input type="hidden" name="id" value={service.id} /><input name="name" defaultValue={service.name} className="h-10 rounded-xl border border-[#e8eaf4] bg-[#fbfcff] px-3 text-sm font-bold text-[#20264f]" /><input name="description" defaultValue={service.description ?? ""} className="h-10 rounded-xl border border-[#e8eaf4] bg-[#fbfcff] px-3 text-sm text-slate-600" /><button className="h-10 rounded-xl border border-[#ded9f3] bg-[#f7f4ff] px-3 text-xs font-black text-[#5d49cc]">حفظ</button></form>) : <div className="rounded-[22px] border border-dashed border-[#dcd8ea] bg-white px-4 py-8 text-center text-sm text-slate-500">لا توجد خدمات بعد. أضف أول خدمة من الأعلى.</div>}</section>
  </div>;
}
