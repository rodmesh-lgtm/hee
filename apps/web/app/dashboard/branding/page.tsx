  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      <section className="rounded-[24px] border border-[#e9e7f3] bg-white p-5">
        ...
        <p className="mt-3 text-[11px] leading-5 text-slate-400">اختيار الثيمات المميزة سيُربط بالمحرر بعد تثبيت الشكل النهائي لكل ثيم. لن نعرض زر اختيار قبل أن يصبح الثيم فعليًا جاهزًا.</p>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <article className="rounded-[24px] border border-[#e9e7f3] bg-white p-5">
          <span className="grid h-11 w-11 place-items-center rounded-2xl bg-blue-50 text-blue-600"><BadgeCheck className="h-5 w-5" /></span>
          <h3 className="mt-3 font-black text-[#1f2552]">توثيق HEE</h3>
          <p className="mt-2 text-xs leading-6 text-slate-500">{business.isVerified ? "تم اعتماد المنشأة وتظهر شارة التوثيق في الصفحة العامة." : verificationPending ? "طلبك قيد المراجعة. ستظهر الشارة فقط بعد اعتماد HEE." : entitlements.verificationEligible ? "باقتك مؤهلة. أرسل طلب التوثيق وسيتم وضعه في قائمة المراجعة." : "التوثيق متاح في الباقات المدفوعة، وبعد الترقية يمكن تقديم الطلب للمراجعة."}</p>
          {!business.isVerified && entitlements.verificationEligible && !verificationPending ? <form action={requestVerificationAction} className="mt-4"><button className="inline-flex h-10 items-center gap-2 rounded-xl bg-blue-600 px-4 text-xs font-black text-white"><Send className="h-3.5 w-3.5" /> طلب التوثيق</button></form> : null}
          {!entitlements.verificationEligible ? <Link href="/dashboard/branding" className="mt-4 inline-flex text-xs font-black text-[#6543ce]">يتطلب ترقية الباقة</Link> : null}
        </article>
      ...
      <section className="rounded-[24px] border border-[#e6e1fb] bg-[#f8f5ff] p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-[#6642cf]" /><h2 className="font-black text-[#1f2552]">نظام الباقات جزء من صلاحيات الحساب</h2></div><p className="mt-1 text-xs leading-6 text-slate-500">الحدود المعروضة هنا هي نفسها التي يطبقها السيرفر على الفروع والأقسام وفريق التواصل، وسنربط بقية الميزات بنفس المصدر.</p></div><Link href="/dashboard/settings" className="inline-flex h-10 items-center justify-center rounded-xl border border-[#dcd5f7] bg-white px-4 text-sm font-black text-[#5b3fd6]">تفاصيل الحساب</Link></div></section>
    </div>
  );
}
