...
export default async function DashboardBrandingPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4 pb-4">
    ...
    {upgradeParam === "billing-unavailable" ? <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold text-amber-800">الاشتراك المدفوع لم يُفتح بعد. لن نفعّل باقة مدفوعة قبل اكتمال مسار دفع آمن ومؤكد.</div> : null}
    ...
    {upgradeParam && !["current", "billing-unavailable", "unavailable", "pending"].includes(upgradeParam) ? <div className="rounded-2xl border border-[#d9cff8] bg-[#f7f4ff] px-4 py-3 text-sm font-bold text-[#5638b8]">تم تسجيل طلب الترقية إلى {upgradeParam.toUpperCase()}.</div> : null}
