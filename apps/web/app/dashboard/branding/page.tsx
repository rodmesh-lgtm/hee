...
export default async function DashboardBrandingPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="grid gap-3 lg:grid-cols-2">
      <article className="rounded-[24px] border border-[#e7e9f4] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><Crown className="h-5 w-5 text-[#6f3bd2]" /><h2 className="font-black text-[#20264f]">باقتك: {business.plan?.name || "Free"}</h2></div><p className="mt-2 text-xs leading-6 text-slate-500">الترقية ترفع حدود الفروع والفريق وتتيح طلب التوثيق، ثم تفتح الثيمات المدفوعة عند اعتمادها.</p><div className="mt-4 flex flex-wrap gap-2">{currentPlan === "FREE" ? <form action={requestPlanUpgradeAction}><input type="hidden" name="plan" value="BUSINESS" /><button className="min-h-11 rounded-xl bg-[#6f3bd2] px-4 text-xs font-black text-white">طلب Business</button></form> : null}{currentPlan === "BUSINESS" ? <form action={requestPlanUpgradeAction}><input type="hidden" name="plan" value="PRO" /><button className="min-h-11 rounded-xl bg-[#6f3bd2] px-4 text-xs font-black text-white">طلب Pro</button></form> : null}{currentPlan === "PRO" ? <span className="inline-flex min-h-11 items-center rounded-xl bg-emerald-50 px-4 text-xs font-black text-emerald-700">أعلى باقة مفعلة</span> : null}<Link href="/dashboard/settings" className="inline-flex min-h-11 items-center rounded-xl border border-[#e5e1f0] px-4 text-xs font-black text-slate-600">تفاصيل الحساب</Link></div></article>
