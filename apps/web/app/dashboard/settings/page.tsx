...
function billingAlert(code: string | undefined) {
  ...
  if (code === "pending") {
    return <div role="status" className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-3 text-sm font-bold leading-7 text-blue-950"><b>عملية الدفع ما زالت قيد التحقق.</b><br />لا تعِد الدفع الآن. ستواصل iR مطابقة حالة العملية مع مزود الدفع، ويمكنك متابعة النتيجة من إدارة الاشتراك والفوترة.</div>;
  }
  if (code === "payment-reversed") {
    return <div role="alert" className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-sm font-bold leading-7 text-amber-950"><b>لم يتم تفعيل الاشتراك المدفوع، وتم طلب عكس عملية الدفع تلقائيًا.</b><br />أثبتت بوابة الدفع العملية، لكن شروط تفعيل الاشتراك لم تعد مكتملة عند التسوية، لذلك لم تحتفظ iR بالمبلغ مقابل اشتراك غير مفعّل. قد يستغرق ظهور الإلغاء أو الاسترداد في كشف البطاقة المدة التي يحددها البنك ومقدم الدفع. إذا بقيت العملية ظاهرة كمبلغ نهائي بعد مدة المعالجة البنكية، افتح طلبًا من {supportLink()}.</div>;
  }
