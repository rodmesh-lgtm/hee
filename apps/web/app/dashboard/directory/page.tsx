...
const STATUS_MESSAGES: Record<string, { tone: "success" | "error"; text: string }> = {
  ...
  "error-too-long": { tone: "error", text: "أحد الحقول أطول من الحد المسموح. اختصر النص ثم حاول مرة أخرى." },
  "error-map": { tone: "error", text: "استخدم رابط Google Maps صالحًا للفرع." },
  "error-email": { tone: "error", text: "أدخل بريدًا إلكترونيًا صالحًا لعضو الفريق." },
...
export default async function DirectoryPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="rounded-[24px] border border-[#e7e9f4] bg-white p-4 sm:p-5">
      ...
      {contactLimitReached ? <div className="mt-3 rounded-xl bg-[#f7f4ff] px-3 py-2 text-xs font-bold text-[#5d49cc]">استخدمت حد أعضاء الفريق. <Link href="/dashboard/settings" className="underline">عرض الباقات</Link></div> : <form action={createContactPersonAction} className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><input name="name" required maxLength={120} placeholder="الاسم" className={field} /><input name="jobTitle" maxLength={120} placeholder="المسمى الوظيفي" className={field} /><input name="whatsapp" maxLength={40} placeholder="واتساب" dir="ltr" className={field} /><input name="phone" maxLength={40} placeholder="الهاتف (اختياري)" dir="ltr" className={field} /><select name="branchId" className={field}><option value="">كل الفروع / بدون فرع</option>{business.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select><input name="email" type="email" maxLength={254} placeholder="البريد (اختياري)" dir="ltr" className={field} /><button className={`${save} sm:w-fit`}><Plus className="h-4 w-4" />إضافة عضو</button></form>}
      <div className="mt-4 space-y-2">{business.contactPersons.length ? business.contactPersons.map((contact) => <details key={contact.id} className="rounded-2xl border border-[#eceef6] bg-[#fbfcff] p-3 open:bg-white"><summary className="flex cursor-pointer list-none items-center justify-between gap-3"><div className="min-w-0"><b className="block truncate text-sm text-[#252a4a]">{contact.name}</b><span className="mt-0.5 block truncate text-[10px] text-slate-400">{contact.jobTitle || "فريق التواصل"}{contact.branch ? ` · ${contact.branch.name}` : ""}</span></div><span className="inline-flex items-center gap-1 text-[10px] font-black text-[#5d49cc]"><Pencil className="h-3.5 w-3.5" />تعديل</span></summary><form action={updateContactPersonAction} className="mt-3 grid gap-2 border-t border-[#eef0f6] pt-3 sm:grid-cols-2 lg:grid-cols-4"><input type="hidden" name="id" value={contact.id} /><input type="hidden" name="sortOrder" value={contact.sortOrder} /><input name="name" required maxLength={120} defaultValue={contact.name} className={field} /><input name="jobTitle" maxLength={120} defaultValue={contact.jobTitle ?? ""} className={field} /><input name="whatsapp" maxLength={40} defaultValue={contact.whatsapp ?? ""} dir="ltr" className={field} /><input name="phone" maxLength={40} defaultValue={contact.phone ?? ""} dir="ltr" className={field} /><select name="branchId" defaultValue={contact.branchId ?? ""} className={field}><option value="">كل الفروع / بدون فرع</option>{business.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select><input name="email" type="email" maxLength={254} defaultValue={contact.email ?? ""} dir="ltr" className={field} /><label className="flex h-10 items-center gap-2 text-xs font-bold text-slate-600"><input type="checkbox" name="isPrimary" defaultChecked={contact.isPrimary} />رئيسي</label><label className="flex h-10 items-center gap-2 text-xs font-bold text-slate-600"><input type="checkbox" name="isActive" defaultChecked={contact.isActive} />ظاهر</label><button className={save}><Save className="h-3.5 w-3.5" />حفظ</button></form><form action={deleteContactPersonAction} className="mt-2"><input type="hidden" name="id" value={contact.id} /><ConfirmSubmitButton confirmMessage={`حذف «${contact.name}» من فريق التواصل؟`} /></form></details>) : <p className="rounded-2xl border border-dashed border-[#e1deeb] p-5 text-center text-xs text-slate-400">لا يوجد أعضاء فريق بعد.</p>}</div>
    </section>
  </div>;
}