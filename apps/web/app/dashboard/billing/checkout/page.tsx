...
export default async function BillingCheckoutPage({ searchParams }: { searchParams: Promise<{ billing?: string }> }) {
  ...
  if (!["created", "initiated", "failed"].includes(billing.status)) redirect("/dashboard/settings?billing=unavailable");
  ...
  return <div className="mx-auto max-w-2xl space-y-4 pb-8">
    ...
    <section className="rounded-[24px] border border-[#e7e4f0] bg-white p-5">
      ...
      <p className="mt-4 rounded-2xl bg-[#faf9fd] p-3 text-xs leading-6 text-slate-600">{upgrade ? "هذا المبلغ محسوب للمدة المتبقية من اشتراكك الحالي، ولن تدفع سعر شهر كامل مرتين." : `سعر الباقة ${billing.monthlyPrice} ر.س شهريًا. عند حفظ وسيلة الدفع بنجاح يمكن تجديد الاشتراك تلقائيًا، ويمكنك إيقاف التجديد من إعدادات الحساب.`}</p>
    </section>
