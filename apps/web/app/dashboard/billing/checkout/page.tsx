...
export default async function BillingCheckoutPage({ searchParams }: { searchParams: Promise<{ billing?: string }> }) {
  ...
  if (!billing || billing.businessId !== business.id) redirect("/dashboard/branding?billing=invalid");
  ...
  return <div className="mx-auto max-w-2xl space-y-4 pb-8">
    <section className="rounded-[26px] border border-[#e7e4f0] bg-white p-5 sm:p-6">
      <div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#f1edff] text-[#6543ce]"><CreditCard className="h-5 w-5" /></span><div><h1 className="text-xl font-black text-[#1f2552]">الدفع والاشتراك</h1><p className="mt-1 text-sm leading-6 text-slate-500">راجع المبلغ وشروط التجديد قبل فتح نموذج ميسر. لا تمر بيانات البطاقة عبر خوادم INFRO.</p></div></div>
    </section>

    <section className="rounded-[24px] border border-[#e7e4f0] bg-white p-5">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><span className="text-xs font-bold text-slate-400">{upgrade ? "ترقية إلى" : "الاشتراك في"}</span><h2 className="mt-1 text-2xl font-black text-[#1f2552]">{billing.planName}</h2></div><div className="text-left" dir="rtl"><span className="text-xs text-slate-400">المبلغ المستحق الآن</span><div className="mt-1 text-2xl font-black text-[#5b3fd6]">{amountSar} ر.س</div></div></div>
    ...
    <section className="rounded-[24px] border border-[#e7e4f0] bg-white p-4 sm:p-5">
      ...
      {staleProviderlessCheckout ? <div role="alert" className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-7 text-amber-950"><b>انتهت صلاحية رابط الدفع القديم قبل بدء أي عملية لدى ميسر.</b><br />لن نسمح باستخدام سعر أو تفاصيل دفع قديمة. ارجع إلى الباقات وأنشئ عملية دفع جديدة بالمعلومات الحالية.<div className="mt-3"><Link href="/dashboard/branding" className="inline-flex min-h-11 items-center rounded-xl border border-amber-300 bg-white px-4 text-xs font-black text-amber-950">العودة إلى الباقات</Link></div></div> : providerStarted ? <div role="status" className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-7 text-amber-900"><b>عملية الدفع قيد التحقق.</b><br />لن نعرض نموذج دفع جديد لنفس العملية حتى لا يحدث خصم مكرر. إذا أكملت التحقق البنكي فستتحدث حالة اشتراكك تلقائيًا.{reconcileUrl ? <div className="mt-3"><Link href={reconcileUrl} prefetch={false} className="inline-flex min-h-11 items-center rounded-xl border border-amber-300 bg-white px-4 text-xs font-black text-amber-950">التحقق من حالة العملية الآن</Link></div> : null}</div> : configured ? <MoyasarCheckout amount={billing.amount} publishableKey={moyasarPublishableKey()} callbackUrl={callbackUrl} billingId={billing.id} businessId={billing.businessId} description={`INFRO ${billing.planName} subscription`} /> : <div role="alert" className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-7 text-amber-900"><b>الدفع غير مفعّل في هذه البيئة.</b><br />لن يتم تحصيل أي مبلغ حتى تُضاف مفاتيح ميسر المعتمدة للبيئة الحالية.</div>}
    </section>
