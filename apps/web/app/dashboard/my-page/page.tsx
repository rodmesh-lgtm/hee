...
import { AlertCircle, BarChart3, Building2, CheckCircle2, ChevronDown, ClipboardList, Crown, Eye, MapPin, MessageCircle, Package, Pencil, Phone, Tag, Users } from "lucide-react";
  ...
  if (needsModuleMigration) {
    await db.business.update({
      ...
      data: {
        ...
        bookingAvailable: normalizedModules.some((pageModule) => pageModule.enabled && ["request", "services", "inquiry"].includes(pageModule.id)),
        acceptOnlineOrders: normalizedModules.some((pageModule) => pageModule.enabled && pageModule.id === "products"),
      },
    });
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {business.isPublished ? (
              ...
              <Link href="/dashboard/my-page?edit=1" className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#5b3fd6] px-4 text-sm font-bold text-white"><Pencil className="h-4 w-4" /> تجهيز الصفحة للنشر</Link>
            )}
          </div>
        </div>
      </section>
        ...
        <aside className="space-y-3">
          <div className="rounded-[28px] border border-[#d9deeb] bg-white p-3 shadow-[0_35px_50px_-36px_rgba(31,37,82,0.65)]">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
              ...
              {business.isPublished ? <a href={publicUrl} target="_blank" rel="noreferrer" className="rounded-xl border border-[#e8eaf5] bg-white px-3 py-2 text-center font-bold text-slate-600">فتح الصفحة</a> : <Link href="/dashboard/my-page?edit=1" className="rounded-xl border border-[#e8eaf5] bg-white px-3 py-2 text-center font-bold text-slate-600">تعديل الصفحة</Link>}
            </div>
          </div>

          <div className="rounded-2xl border border-[#edf0fb] bg-white p-4"><div className="flex items-center justify-between"><h3 className="text-sm font-black text-[#1f2552]">تابع أداءك</h3><BarChart3 className="h-4 w-4 text-[#5b4fd8]" /></div><p className="mt-2 text-xs leading-6 text-slate-500">انتقل إلى صفحة الأداء لمعرفة كيف يتفاعل العملاء مع صفحتك.</p><Link href="/dashboard/analytics" className="mt-3 inline-flex w-full items-center justify-center rounded-xl border border-[#ddd7ff] bg-[#f4f1ff] px-3 py-2 text-sm font-bold text-[#4f43d9]">عرض الأداء</Link></div>
        </aside>
        ...
        <div className="space-y-4">
          <section className="rounded-2xl border border-[#edf0fb] bg-white p-4 sm:p-5 shadow-[0_18px_32px_-28px_rgba(40,50,110,0.42)]">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-[152px_minmax(0,1fr)]">
              <div className="overflow-hidden rounded-2xl border border-[#e8ebf7] bg-[#f8faff]">{business.coverUrl ? <Image src={business.coverUrl} alt={business.name} width={600} height={400} unoptimized className="h-[152px] w-full object-cover" /> : <div className="flex h-[152px] items-center justify-center text-xs text-slate-400">غلاف الصفحة</div>}</div>
              <div className="space-y-3">
                ...
                <div className="grid gap-2 sm:grid-cols-3">
                  <Link href="/dashboard/my-page?edit=1" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#5b3fd6] px-4 text-sm font-bold text-white"><Pencil className="h-4 w-4" /> تعديل صفحتي</Link>
                  {business.isPublished ? <a href={publicUrl} target="_blank" rel="noreferrer" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-[#ded8ff] bg-[#f7f5ff] px-4 text-sm font-bold text-[#4f43d9]"><Eye className="h-4 w-4" /> عرض الصفحة</a> : <Link href="/dashboard/directory" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 text-sm font-bold text-amber-700"><Users className="h-4 w-4"/> إكمال بيانات التواصل</Link>}
          ...
          <section className={`rounded-2xl border p-4 sm:p-5 ${directoryReady ? "border-emerald-100 bg-emerald-50/40" : "border-amber-100 bg-amber-50/50"}`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Link href="/dashboard/directory" className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-[#dcd8ef] bg-white px-4 text-sm font-black text-[#4f43d9]"><Pencil className="h-4 w-4"/> إدارة الدليل</Link>
            </div>
          ...
          <section className="rounded-2xl border border-[#edf0fb] bg-white p-4 sm:p-5 shadow-[0_18px_32px_-28px_rgba(40,50,110,0.42)]">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <Link href="/dashboard/branding" className="rounded-2xl border border-[#edf0fb] bg-white px-4 py-4 transition hover:border-[#ddd7ff] hover:bg-[#faf8ff]"><Tag className="h-5 w-5 text-[#5b4fd8]" /><p className="mt-3 text-sm font-black text-[#1f2552]">تغيير الصور</p><p className="mt-1 text-xs text-slate-500">حدّث صور صفحتك بسهولة</p></Link>
              <Link href="/dashboard/directory" className="rounded-2xl border border-[#edf0fb] bg-white px-4 py-4 transition hover:border-[#ddd7ff] hover:bg-[#faf8ff]"><Building2 className="h-5 w-5 text-[#0f9f70]" /><p className="mt-3 text-sm font-black text-[#1f2552]">الفروع والتواصل</p><p className="mt-1 text-xs text-slate-500">أدر الفروع والأقسام والمسؤولين</p></Link>
              <Link href="/dashboard/business" className="rounded-2xl border border-[#edf0fb] bg-white px-4 py-4 transition hover:border-[#ddd7ff] hover:bg-[#faf8ff]"><MapPin className="h-5 w-5 text-[#5b4fd8]" /><p className="mt-3 text-sm font-black text-[#1f2552]">بيانات النشاط</p><p className="mt-1 text-xs text-slate-500">حدّث عنوان وبيانات نشاطك</p></Link>
              <Link href="/dashboard/contact-links" className="rounded-2xl border border-[#edf0fb] bg-white px-4 py-4 transition hover:border-[#ddd7ff] hover:bg-[#faf8ff]"><Phone className="h-5 w-5 text-[#2a9d59]" /><p className="mt-3 text-sm font-black text-[#1f2552]">روابط التواصل</p><p className="mt-1 text-xs text-slate-500">حدّث واتساب وروابط التواصل</p></Link>
              <Link href="/dashboard/offers" className="rounded-2xl border border-[#edf0fb] bg-white px-4 py-4 transition hover:border-[#ddd7ff] hover:bg-[#faf8ff]"><Tag className="h-5 w-5 text-[#ff5a5a]" /><p className="mt-3 text-sm font-black text-[#1f2552]">إضافة عرض</p><p className="mt-1 text-xs text-slate-500">أنشئ عرضاً جذاباً لعملائك</p></Link>
              <Link href="/dashboard/catalog" className="rounded-2xl border border-[#edf0fb] bg-white px-4 py-4 transition hover:border-[#ddd7ff] hover:bg-[#faf8ff]"><Package className="h-5 w-5 text-[#9a3fe0]" /><p className="mt-3 text-sm font-black text-[#1f2552]">الخدمات والمنتجات</p><p className="mt-1 text-xs text-slate-500">أدر ما تقدمه لعملائك</p></Link>
            </div>
            <div className="mt-4 rounded-2xl border border-[#e6defe] bg-gradient-to-l from-[#f3efff] to-[#fbf9ff] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-base font-black text-[#2a2f61]">أضف عرضاً مميزاً هذا الأسبوع لزيادة التفاعل والطلبات.</p><Link href="/dashboard/offers" className="inline-flex h-10 items-center justify-center rounded-xl border border-[#d9d1ff] bg-white px-4 text-sm font-bold text-[#4f43d9]">إنشاء عرض الآن</Link></div></div>
          </section>
        </div>

        <aside className="space-y-3">
          <section className="rounded-2xl border border-[#edf0fb] bg-white p-4 text-center"><div className="mx-auto inline-flex h-11 w-11 items-center justify-center rounded-full bg-[#f3efff] text-[#5a4ed7]"><Crown className="h-5 w-5" /></div><h3 className="mt-3 text-xl font-black text-[#1f2552]">طوّر نشاطك</h3><p className="mt-2 text-sm text-slate-500">ميزات أكثر وتخصيصات أقوى مع الباقات المدفوعة.</p><Link href="/dashboard/settings" className="mt-4 inline-flex h-10 w-full items-center justify-center rounded-xl border border-[#ddd7ff] bg-[#f4f1ff] px-4 text-sm font-bold text-[#4f43d9]">عرض الباقات</Link></section>
