...
const ACCOUNT_DELETION_CONFIRMATION = "DELETE MY INFRO ACCOUNT";
...
export default async function AccountDeletionPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-5 pb-8">
    ...
    <section className="overflow-hidden rounded-[28px] border border-rose-200 bg-white"><div className="border-b border-rose-100 bg-rose-50/70 p-5"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-rose-100 text-rose-700"><Trash2 className="h-5 w-5" /></span><div><span className="text-[10px] font-black tracking-[.18em] text-rose-700">FINAL AUTHORIZATION</span><h2 className="mt-1 font-black text-rose-950">التأكيد النهائي</h2><p className="mt-1 text-xs leading-6 text-slate-600">لن ينفذ INFRO الحذف إلا بعد اجتياز حواجز الهوية أدناه. عبارة التأكيد ثابتة ومطابقة لهوية INFRO الحالية.</p></div></div></div>
