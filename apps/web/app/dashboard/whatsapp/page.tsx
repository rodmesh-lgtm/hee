...
import { ContactRound, FileText, Link2, MessageCircle, Megaphone, ShieldCheck } from "lucide-react";
...
export default async function WhatsAppMarketingPage() {
  ...
  const [connection, contacts, templates, campaigns, conversations] = await Promise.all([
  ...
  return <div className="space-y-4 pb-5">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <Metric label="جهات الاتصال" value={String(contacts)} /><Metric label="قوالب معتمدة" value={String(templates)} /><Metric label="الحملات" value={String(campaigns)} /><Metric label="المحادثات" value={String(conversations)} />
    </section>
