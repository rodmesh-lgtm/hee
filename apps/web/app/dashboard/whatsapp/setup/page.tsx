...
export default async function WhatsAppSetupPage() {
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      <article className="rounded-[24px] border border-[#e7e9f4] bg-white p-5"><h2 className="font-black text-[#20264f]">حالة الاتصال</h2>{connection ? <div className="mt-4 space-y-3"><p className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700"><CheckCircle2 className="h-4 w-4" />{connection.status === "connected" ? "متصل وفعّال" : connection.status}</p><dl className="grid gap-2 text-sm"><div><dt className="text-xs text-slate-400">الاسم الموثق</dt><dd className="font-bold text-[#303653]">{connection.verifiedName || "—"}</dd></div><div><dt className="text-xs text-slate-400">الرقم</dt><dd dir="ltr" className="text-right font-bold text-[#303653]">{connection.displayPhoneNumber || "—"}</dd></div><div><dt className="text-xs text-slate-400">WABA / Phone ID</dt><dd dir="ltr" className="break-all text-right text-xs text-slate-500">{connection.wabaId} / {connection.phoneNumberId}</dd></div></dl></div> : <p className="mt-4 text-sm leading-7 text-slate-500">لم يُربط رقم رسمي بهذا النشاط بعد.</p>}</article>
