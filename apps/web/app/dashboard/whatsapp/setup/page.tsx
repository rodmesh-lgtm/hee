...
export default async function WhatsAppSetupPage() {
  ...
  const nextAction = !publicConfig
    ? { title: "خدمة الربط تحتاج تجهيزًا", detail: "Embedded Signup غير متاح في هذه البيئة حاليًا. اعرض حالة الربط أدناه بدل إعادة تحميل الصفحة بلا نتيجة.", href: "#meta-connect", label: "عرض حالة الربط" }
    : !connected
      ? { title: "ربط رقم الشركة رسميًا", detail: "استخدم حساب Meta الذي يدير WABA ورقم WhatsApp Business الخاص بالشركة.", href: "#meta-connect", label: "إكمال الخطوة" }
      : !identityReady
        ? { title: "تحديث بيانات الرقم", detail: "الاتصال موجود لكن الاسم أو الرقم لم يصلا بعد من Meta. أعد تفويض الربط بأمان.", href: "#meta-connect", label: "إكمال الخطوة" }
        : approvedTemplates === 0
          ? { title: "تجهيز قالب للحملة", detail: "الرقم جاهز. الخطوة التالية هي مزامنة قالب معتمد من Meta.", href: "/dashboard/whatsapp/templates", label: "إكمال الخطوة" }
  ...
  return <div className="min-w-0 space-y-5 pb-5">
    <header className="relative overflow-hidden rounded-[30px] bg-[#061719] p-5 text-white shadow-[0_30px_80px_-50px_rgba(3,23,25,.9)] sm:p-7">
      ...
      <div className="relative grid gap-6 xl:grid-cols-[1fr_360px] xl:items-start">
        <div className="min-w-0">
          ...
          <div className="mt-5 rounded-[22px] border border-white/10 bg-white/[.05] p-4"><span className="text-[9px] font-black text-[#6eead8]">NEXT ACTION</span><div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><b className="block text-base">{nextAction.title}</b><span className="mt-1 block text-[10px] leading-5 text-slate-400">{nextAction.detail}</span></div><Link href={nextAction.href} scroll={nextAction.href.startsWith("#") ? true : undefined} className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#35e4cb] px-4 text-[10px] font-black text-[#061719] transition hover:bg-[#65edda] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white motion-reduce:transition-none">{nextAction.label}<ArrowLeft className="h-3.5 w-3.5" /></Link></div></div>
        </div>
    ...
    <section className="grid min-w-0 gap-5 xl:grid-cols-[1.05fr_.95fr]">
      ...
      <article id="meta-connect" className="relative min-w-0 scroll-mt-28 overflow-hidden rounded-[26px] bg-[#07181b] p-4 text-white ring-offset-4 target:ring-2 target:ring-[#35e4cb] sm:p-6">
        ...
        <div className="relative mt-5 rounded-[18px] border border-white/10 bg-white/[.05] p-3 sm:p-4">{publicConfig ? <EmbeddedSignupButton {...publicConfig} /> : <div role="status" className="rounded-xl border border-amber-300/20 bg-amber-300/10 p-3 text-[10px] font-bold leading-6 text-amber-100"><b className="block">خدمة Embedded Signup غير مجهزة في هذه البيئة</b><span className="mt-1 block font-medium text-amber-100/80">زر الربط الرسمي يظهر فقط عندما تتوفر إعدادات Meta العامة المطلوبة. لن تعيد INFRO تحميل الصفحة أو توهمك بأن خطوة ربط بدأت وهي غير متاحة.</span></div>}</div>
