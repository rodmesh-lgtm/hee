...
import { readAutomationTriggerConfig, readTemplateActionConfig, templateHasVariables, WHATSAPP_APPOINTMENT_LEAD_MINUTES, WHATSAPP_AUTOMATION_TRIGGER_TYPES, WHATSAPP_CONFIGURABLE_TRIGGER_TYPES, WHATSAPP_INACTIVE_CUSTOMER_DAYS, WHATSAPP_ORDER_EVENT_STATUSES } from "../../../lib/whatsapp/automation-domain";
...
export default async function WhatsAppAutomationsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className="space-y-4 pb-5">
    ...
    <section className="grid gap-4 lg:grid-cols-[.9fr_1.1fr]">
      <form action={createWhatsAppAutomationAction} className="rounded-[24px] border bg-white p-5"><h2 className="font-black text-[#20264f]">إنشاء أتمتة</h2><p className="mt-2 text-xs leading-6 text-slate-500">تُنشأ كمسودة ولا تبدأ الإرسال حتى تفعيلها. الربط بالرقم يُشتق خادميًا من القالب ولا يؤخذ من المتصفح.</p><label className="mt-4 block text-xs font-bold">اسم الأتمتة<input name="name" required maxLength={120} className="mt-1 h-11 w-full rounded-xl border px-3" placeholder="مثال: تذكير الموعد قبل يوم" /></label><label className="mt-3 block text-xs font-bold">نوع الحدث<select name="triggerType" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر الحدث</option>{WHATSAPP_AUTOMATION_TRIGGER_TYPES.map((trigger) => <option key={trigger} value={trigger} disabled={!configurableTriggers.has(trigger)}>{triggerLabels[trigger]}{configurableTriggers.has(trigger) ? "" : " — غير متاح بعد"}</option>)}</select></label><label className="mt-3 block text-xs font-bold">اسم حدث API (مطلوب لنوع حدث API)<input name="apiEventName" maxLength={64} pattern="[a-z][a-z0-9_.:-]{0,63}" dir="ltr" className="mt-1 h-11 w-full rounded-xl border px-3" placeholder="order.ready" /></label><label className="mt-3 block text-xs font-bold">حالة الطلب المستهدفة (مطلوبة لتحديث الطلب)<select name="orderStatus" className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">لا ينطبق</option>{WHATSAPP_ORDER_EVENT_STATUSES.map((status) => <option key={status} value={status}>{orderStatusLabels[status]}</option>)}</select></label><label className="mt-3 block text-xs font-bold">وقت تذكير الموعد (مطلوب لتذكير الموعد)<select name="reminderLeadMinutes" defaultValue={1440} className="mt-1 h-11 w-full rounded-xl border px-3">{WHATSAPP_APPOINTMENT_LEAD_MINUTES.map((minutes) => <option key={minutes} value={minutes}>{leadLabels[minutes]}</option>)}</select></label><label className="mt-3 block text-xs font-bold">مدة عدم نشاط العميل (مطلوبة للعميل غير النشط)<select name="inactiveDays" defaultValue={90} className="mt-1 h-11 w-full rounded-xl border px-3">{WHATSAPP_INACTIVE_CUSTOMER_DAYS.map((days) => <option key={days} value={days}>{inactiveLabels[days]}</option>)}</select></label><label className="mt-3 block text-xs font-bold">قالب Meta المعتمد<select name="templateId" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر القالب</option>{eligibleTemplates.map((template) => <option key={template.id} value={template.id}>{template.name} · {template.language} · {template.connection.verifiedName || template.connection.displayPhoneNumber || "رقم Meta"}</option>)}</select></label><label className="mt-3 block text-xs font-bold">فترة منع التكرار بالدقائق<input name="cooldownMinutes" type="number" required min={0} max={525600} step={1} defaultValue={1440} className="mt-1 h-11 w-full rounded-xl border px-3" /></label><button disabled={!eligibleTemplates.length} className="mt-4 min-h-11 rounded-xl bg-[#6f3bd2] px-5 text-xs font-black text-white disabled:bg-slate-300">إنشاء كمسودة</button>{!eligibleTemplates.length ? <p className="mt-3 text-xs font-bold text-amber-700">يلزم رقم Meta متصل وقالب Approved بلا متغيرات. مزامنة القوالب متاحة من قسم قوالب Meta.</p> : null}</form>
      <aside className="rounded-[24px] border border-amber-200 bg-amber-50 p-5 text-xs leading-7 text-amber-950"><b className="text-sm">حدود التشغيل الحالية</b><p className="mt-2">التفعيل يجعل الأتمتة تستقبل الأحداث الجديدة فقط؛ لا يُنشئ موافقات ولا يعيد إرسال أحداث قديمة تلقائيًا. الترحيب يعمل عند تسجيل موافقة صريحة جديدة، والمتابعة تعمل بعد اكتمال طلب أو حجز، والعميل غير النشط يُحتسب من آخر طلب أو حجز مكتمل.</p><p className="mt-2">تذكير الموعد يُجدول عند تأكيد الحجز فقط، ولا يُرسل إذا كان وقت التذكير قد فات. حدث API لا يقبل إلا اسم الحدث المضبوط وجهة اتصال قائمة ذات موافقة فعالة. السلة المتروكة تبقى معطلة حتى يتوفر مصدر موثوق. هذه الواجهة تقبل القوالب بلا متغيرات حاليًا.</p><p className="mt-2 font-black">قد تترتب رسوم Meta فعلية عند وصول حدث مؤهل والعامل مفعل. وجود Customer أو Order أو Booking لا يُعد موافقة تسويقية.</p></aside>
    </section>
