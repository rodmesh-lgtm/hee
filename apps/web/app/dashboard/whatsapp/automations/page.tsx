...
import { readTemplateActionConfig, templateHasVariables, WHATSAPP_AUTOMATION_TRIGGER_TYPES } from "../../../lib/whatsapp/automation-domain";
...
export default async function WhatsAppAutomationsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [templates, automations] = await Promise.all([
    ...
    db.whatsAppAutomation.findMany({
      ...
      take: 100,
      select: {
        id: true, name: true, status: true, triggerType: true, actionConfig: true, cooldownMinutes: true,
  ...
  return <div className="space-y-4 pb-5">
    ...
    <section className="grid gap-4 lg:grid-cols-[.9fr_1.1fr]">
      <form action={createWhatsAppAutomationAction} className="rounded-[24px] border bg-white p-5"><h2 className="font-black text-[#20264f]">إنشاء أتمتة</h2><p className="mt-2 text-xs leading-6 text-slate-500">تُنشأ كمسودة ولا تبدأ الإرسال حتى تفعيلها. الربط بالرقم يُشتق خادميًا من القالب ولا يؤخذ من المتصفح.</p><label className="mt-4 block text-xs font-bold">اسم الأتمتة<input name="name" required maxLength={120} className="mt-1 h-11 w-full rounded-xl border px-3" placeholder="مثال: تذكير الموعد قبل الزيارة" /></label><label className="mt-3 block text-xs font-bold">نوع الحدث<select name="triggerType" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر الحدث</option>{WHATSAPP_AUTOMATION_TRIGGER_TYPES.map((trigger) => <option key={trigger} value={trigger}>{triggerLabels[trigger]}</option>)}</select></label><label className="mt-3 block text-xs font-bold">قالب Meta المعتمد<select name="templateId" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر القالب</option>{eligibleTemplates.map((template) => <option key={template.id} value={template.id}>{template.name} · {template.language} · {template.connection.verifiedName || template.connection.displayPhoneNumber || "رقم Meta"}</option>)}</select></label><label className="mt-3 block text-xs font-bold">فترة منع التكرار بالدقائق<input name="cooldownMinutes" type="number" required min={0} max={525600} step={1} defaultValue={1440} className="mt-1 h-11 w-full rounded-xl border px-3" /></label><button disabled={!eligibleTemplates.length} className="mt-4 min-h-11 rounded-xl bg-[#6f3bd2] px-5 text-xs font-black text-white disabled:bg-slate-300">إنشاء كمسودة</button>{!eligibleTemplates.length ? <p className="mt-3 text-xs font-bold text-amber-700">يلزم رقم Meta متصل وقالب Approved بلا متغيرات. مزامنة القوالب متاحة من قسم قوالب Meta.</p> : null}</form>
