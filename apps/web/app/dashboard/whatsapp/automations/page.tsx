...
import { readAutomationTriggerConfig, readTemplateActionConfig, templateHasVariables, WHATSAPP_AUTOMATION_TRIGGER_TYPES, WHATSAPP_ORDER_EVENT_STATUSES } from "../../../lib/whatsapp/automation-domain";
...
export default async function WhatsAppAutomationsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className="space-y-4 pb-5">
    ...
    <section className="grid gap-4 lg:grid-cols-[.9fr_1.1fr]">
      <form action={createWhatsAppAutomationAction} className="rounded-[24px] border bg-white p-5"><h2 className="font-black text-[#20264f]">إنشاء أتمتة</h2><p className="mt-2 text-xs leading-6 text-slate-500">تُنشأ كمسودة ولا تبدأ الإرسال حتى تفعيلها. الربط بالرقم يُشتق خادميًا من القالب ولا يؤخذ من المتصفح.</p><label className="mt-4 block text-xs font-bold">اسم الأتمتة<input name="name" required maxLength={120} className="mt-1 h-11 w-full rounded-xl border px-3" placeholder="مثال: تحديث حالة الطلب" /></label><label className="mt-3 block text-xs font-bold">نوع الحدث<select name="triggerType" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر الحدث</option>{WHATSAPP_AUTOMATION_TRIGGER_TYPES.map((trigger) => <option key={trigger} value={trigger}>{triggerLabels[trigger]}</option>)}</select></label><label className="mt-3 block text-xs font-bold">حالة الطلب المستهدفة (مطلوبة لتحديث الطلب)<select name="orderStatus" className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">لا ينطبق</option>{WHATSAPP_ORDER_EVENT_STATUSES.map((status) => <option key={status} value={status}>{orderStatusLabels[status]}</option>)}</select></label><label className="mt-3 block text-xs font-bold">قالب Meta المعتمد<select name="templateId" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر القالب</option>{eligibleTemplates.map((template) => <option key={template.id} value={template.id}>{template.name} · {template.language} · {template.connection.verifiedName || template.connection.displayPhoneNumber || "رقم Meta"}</option>)}</select></label><label className="mt-3 block text-xs font-bold">فترة منع التكرار بالدقائق<input name="cooldownMinutes" type="number" required min={0} max={525600} step={1} defaultValue={1440} className="mt-1 h-11 w-full rounded-xl border px-3" /></label><button disabled={!eligibleTemplates.length} className="mt-4 min-h-11 rounded-xl bg-[#6f3bd2] px-5 text-xs font-black text-white disabled:bg-slate-300">إنشاء كمسودة</button>{!eligibleTemplates.length ? <p className="mt-3 text-xs font-bold text-amber-700">يلزم رقم Meta متصل وقالب Approved بلا متغيرات. مزامنة القوالب متاحة من قسم قوالب Meta.</p> : null}</form>
      <aside className="rounded-[24px] border border-amber-200 bg-amber-50 p-5 text-xs leading-7 text-amber-950"><b className="text-sm">حدود التشغيل الحالية</b><p className="mt-2">التفعيل يجعل الأتمتة تستقبل الأحداث الجديدة فقط؛ لا يُنشئ موافقات ولا يعيد إرسال أحداث قديمة تلقائيًا. مصادر المواعيد والطلبات والسلال والتكاملات الخارجية يجب أن تُصدر حدثًا دائمًا موثوقًا إلى محرك الأتمتة.</p><p className="mt-2">هذه الواجهة تقبل القوالب بلا متغيرات حاليًا. القوالب ذات المتغيرات تتطلب Workflow mapping صريحًا حتى لا تُرسل بيانات ناقصة أو من tenant مختلف.</p><p className="mt-2 font-black">قد تترتب رسوم Meta فعلية عند وصول حدث مؤهل والعامل مفعل. وجود Customer أو Order أو Booking لا يُعد موافقة تسويقية.</p></aside>
    </section>
    <section className="space-y-3">{automations.map((automation) => { const template = templatesById.get(templateIdFromConfig(automation.actionConfig) || ""); const lastRun = automation.runs[0]; const detail = triggerDetail(automation.triggerType, automation.triggerConfig); return <article key={automation.id} className="rounded-[24px] border bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><b className="text-[#20264f]">{automation.name}</b><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${automation.status === "active" ? "bg-emerald-50 text-emerald-700" : automation.status === "paused" ? "bg-amber-50 text-amber-700" : "bg-[#f2eeff] text-[#6543ce]"}`}>{statusLabels[automation.status] || automation.status}</span></div><span className="mt-1 block text-xs text-slate-400">{triggerLabels[automation.triggerType as keyof typeof triggerLabels] || automation.triggerType}{detail ? ` (${detail})` : ""} · {template ? `${template.name} · ${template.language}` : "القالب غير متاح"} · {automation.connection.verifiedName || automation.connection.displayPhoneNumber || "رقم Meta"}</span></div><div className="flex flex-wrap gap-2">{automation.status === "draft" ? <AutomationButton id={automation.id} operation="activate" label="تفعيل" confirm /> : null}{automation.status === "active" ? <AutomationButton id={automation.id} operation="pause" label="إيقاف مؤقت" /> : null}{automation.status === "paused" ? <AutomationButton id={automation.id} operation="resume" label="استئناف" confirm /> : null}</div></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Mini label="فترة المنع" value={`${automation.cooldownMinutes} دقيقة`} /><Mini label="مرات التشغيل" value={String(automation._count.runs)} /><Mini label="وظائف الإرسال" value={String(automation._count.jobs)} /><Mini label="آخر تشغيل" value={lastRun ? `${lastRun.status} · ${lastRun.createdAt.toLocaleString("ar-SA")}` : "لا يوجد"} /></div>{lastRun?.skipReason ? <p className="mt-3 rounded-xl bg-slate-50 px-3 py-2 text-[10px] text-slate-500">سبب آخر تخطٍ: {lastRun.skipReason}</p> : null}</article>; })}{!automations.length ? <div className="rounded-[24px] border border-dashed bg-white p-10 text-center text-sm text-slate-400"><Activity className="mx-auto mb-2 h-7 w-7" />لا توجد أتمتة بعد.</div> : null}</section>
  </div>;
}
