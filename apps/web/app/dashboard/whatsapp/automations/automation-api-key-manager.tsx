...
export function AutomationApiKeyManager({ keys }: { keys: SafeKey[] }) {
  ...
  return <section className="rounded-[24px] border bg-white p-5">
    ...
    <div className="mt-4 space-y-2">{keys.map((key) => <div key={key.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-slate-50 p-3"><div><b className="text-xs text-[#20264f]">{key.name}</b><code dir="ltr" className="ms-2 text-[10px] text-slate-500">{key.keyPrefix}…</code><span className={`ms-2 rounded-full px-2 py-1 text-[9px] font-black ${key.status === "active" ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-600"}`}>{key.status === "active" ? "نشط" : "ملغي"}</span><span className="mt-1 block text-[10px] text-slate-400">آخر استخدام: {key.lastUsedAt ? new Date(key.lastUsedAt).toLocaleString("ar-SA") : "لم يُستخدم"}</span></div>{key.status === "active" ? <form action={revokeWhatsAppAutomationApiKeyAction}><input type="hidden" name="keyId" value={key.id} /><button className="rounded-lg bg-rose-50 px-3 py-2 text-[10px] font-black text-rose-700">إلغاء المفتاح</button></form> : null}</div>)}{!keys.length ? <p className="text-xs text-slate-400">لا توجد مفاتيح بعد.</p> : null}</div>
