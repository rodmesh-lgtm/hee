...
export default async function WhatsAppCareOperationsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="min-w-0 space-y-5 pb-5">
    <header className="relative overflow-hidden rounded-[28px] bg-[#07181b] p-5 text-white shadow-[0_28px_80px_-52px_rgba(3,23,25,.9)] sm:p-7">
      ...
      <div className="relative flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-2 text-xs leading-7 text-slate-300">رتّب المحادثات بحسب المسؤول والأولوية وموعد الاستجابة، مع إبقاء محتوى الرسائل داخل صندوق المحادثة فقط.</p>
        </div>
    ...
    <section className="grid grid-cols-2 gap-2 lg:grid-cols-3 xl:grid-cols-6">
      ...
      <Metric label="متجاوزة SLA" value={operations.summary.overdue} helper="انتهت مهلة الاستجابة" attention />
      <Metric label="غير معيّنة" value={operations.summary.unassigned} helper="بلا موظف مسؤول" />
          ...
          return <article key={item.id} className="rounded-[20px] border border-slate-200 bg-[#fbfdfd] p-4 transition hover:border-[#bcebe5] hover:shadow-[0_14px_36px_-30px_rgba(7,24,27,.45)]">
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2"><b className="truncate text-sm text-slate-950">{item.customerDisplayName || "عميل واتساب"}</b>{item.needsReply ? <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[9px] font-black text-amber-700">تحتاج متابعة</span> : <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[9px] font-black text-slate-500">لا يوجد وارد أحدث</span>}<span className="rounded-full bg-blue-50 px-2.5 py-1 text-[9px] font-black text-blue-700">{item.priority === "urgent" ? "عاجلة" : item.priority === "high" ? "عالية" : item.priority === "low" ? "منخفضة" : "عادية"}</span>{item.sla.overdue ? <span className="rounded-full bg-rose-50 px-2.5 py-1 text-[9px] font-black text-rose-700">SLA متأخر</span> : null}</div>
                <span dir="ltr" className="mt-1 block text-right text-[10px] text-slate-400">{item.customerPhoneE164}</span>
                <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-[9px] text-slate-500"><span>المسؤول: {item.assignee?.name ?? "غير معيّنة"}</span><span>موعد SLA: {formatDate(item.sla.dueAt)}</span><span>آخر وارد: {formatDate(item.lastInboundAt)}</span><span>آخر صادر: {formatDate(item.lastOutboundAt)}</span><span>آخر نشاط: {formatDate(item.lastMessageAt)}</span>{latest ? <span>آخر رسالة: {latest.direction === "inbound" ? "واردة" : "صادرة"}</span> : null}</div>
              </div>
    ...
    <section className="rounded-[20px] border border-slate-200 bg-white p-4"><p className="flex items-start gap-2 text-[10px] leading-6 text-slate-500"><ShieldCheck className="mt-1 h-4 w-4 shrink-0 text-[#008f87]" /><span>التعيين محصور بمالك المنشأة وأعضاء خدمة العملاء النشطين داخل المنشأة نفسها. تسجّل تغييرات المسؤول والأولوية في سجل التدقيق.</span></p></section>
  </div>;
}
