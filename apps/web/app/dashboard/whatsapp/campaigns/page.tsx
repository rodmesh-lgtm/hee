...
export default async function WhatsAppCampaignsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [connections, templates, campaigns, eligibleConsents, launchReadiness] = await Promise.all([
    db.whatsAppConnection.findMany({ where: { businessId: context.businessId, provider: "meta", status: "connected" }, select: { id: true, verifiedName: true, displayPhoneNumber: true } }),
    db.whatsAppTemplate.findMany({ where: { businessId: context.businessId, status: "approved" }, select: { id: true, connectionId: true, name: true, language: true, category: true }, orderBy: { name: "asc" } }),
  ...
  const operationSucceeded = ["launch", "schedule", "pause", "resume", "cancel"].includes(params.operation || "");
  const operationMessage = params.operation === "worker-unavailable"
    ? `الإطلاق متوقف بأمان: ${readinessLabel(launchReadiness)}. لم تُضف أي رسائل جديدة إلى طابور الإرسال.`
    : params.operation && params.operation !== "failed"
      ? `تم تنفيذ ${params.operation} بأمان.`
      : "تعذرت العملية؛ تحقق من الاتصال والقالب والموافقات وحالة الحملة.";
  ...
  return <div className="space-y-4 pb-5">
    ...
    <section className={`rounded-[20px] border p-4 ${launchReadiness.ready ? "border-emerald-200 bg-emerald-50" : "border-amber-200 bg-amber-50"}`}><div className="flex items-start gap-3"><Activity className={`mt-0.5 h-5 w-5 shrink-0 ${launchReadiness.ready ? "text-emerald-600" : "text-amber-600"}`} /><div><b className="text-sm text-[#20264f]">{launchReadiness.ready ? "عامل الإرسال جاهز للإطلاق" : "الإطلاق الفوري مقفل مؤقتًا"}</b><p className="mt-1 text-xs leading-6 text-slate-600">{launchReadiness.ready ? `آخر دورة تشغيل ناجحة: ${launchReadiness.lastSucceededAt.toLocaleString("ar-SA")}. سيعاد فحص الموافقة وOpt-out والاتصال قبل كل رسالة.` : `${readinessLabel(launchReadiness)}. يمكنك تجهيز الحملة، لكن زر الإطلاق لن يضيف مستلمين للطابور حتى تعود دورة التشغيل إلى حالة سليمة.`}</p></div></div></section>
    {params.create || params.operation ? <p className={`rounded-2xl p-3 text-xs font-bold ${(params.create === "complete" || operationSucceeded) ? "bg-emerald-50 text-emerald-800" : "bg-rose-50 text-rose-800"}`}>{params.create === "complete" ? "أُنشئت الحملة وأُخذت لقطة المستلمين المؤهلين. راجع العدد قبل الإطلاق." : operationMessage}</p> : null}
    <section className="grid gap-4 lg:grid-cols-[.8fr_1.2fr]"><form action={createWhatsAppCampaignAction} className="rounded-[24px] border bg-white p-5"><h2 className="font-black text-[#20264f]">إنشاء حملة</h2><p className="mt-2 text-xs leading-6 text-slate-500">سيُضمّن فقط أصحاب الموافقة الفعالة وغير المنسحبين. المتاح حاليًا: {eligibleConsents} موافقة.</p><label className="mt-4 block text-xs font-bold">اسم الحملة<input name="name" required maxLength={120} className="mt-1 h-11 w-full rounded-xl border px-3" /></label><label className="mt-3 block text-xs font-bold">الرقم الرسمي<select name="connectionId" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر الرقم</option>{connections.map((item) => <option key={item.id} value={item.id}>{item.verifiedName || item.displayPhoneNumber || item.id}</option>)}</select></label><label className="mt-3 block text-xs font-bold">القالب المعتمد<select name="templateId" required className="mt-1 h-11 w-full rounded-xl border px-3"><option value="">اختر القالب</option>{templates.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.language} · {item.category}</option>)}</select></label><button disabled={!connections.length || !templates.length || !eligibleConsents} className="mt-4 min-h-11 rounded-xl bg-[#6f3bd2] px-5 text-xs font-black text-white disabled:bg-slate-300">إنشاء وأخذ Snapshot</button>{!connections.length ? <Link href="/dashboard/whatsapp/setup" className="mt-3 block text-xs font-bold text-emerald-700">اربط رقم WhatsApp Business أولًا ←</Link> : null}</form>
      <div className="rounded-[24px] border border-amber-200 bg-amber-50 p-5 text-xs leading-7 text-amber-950"><b className="text-sm">اختبار الإطلاق الحقيقي</b><p className="mt-2">زر الإطلاق يضيف المستلمين إلى الطابور فقط بعد إثبات أن عامل WhatsApp أتم دورة تشغيل حديثة بنجاح. العامل يفحص الاشتراك والاتصال والموافقة وOpt-out مرة أخرى قبل كل رسالة، ثم يستخدم قالب Meta المعتمد. قد تنتج رسوم Meta فعلية عند تشغيل العامل.</p><p className="mt-2 font-bold">لا تختبر على أرقام بلا موافقة، ولا تستخدم قائمة عملاء أو طلبات كبديل للموافقة الصريحة.</p></div></section>
