...
export function MessageComposer({ components, value, onChange }: { components: unknown; value: Composition; onChange: (value: Composition) => void }) {
  ...
  try { preview = resolveCampaignComposition(components, value, { displayName: "عميل تجريبي", phoneE164: "+966500000000", email: "example@example.com" }).values; } catch { /* Required fields remain visible below. */ }
  ...
  return <section className="space-y-4" aria-label="تخصيص رسالة الحملة">
    ...
    <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-7 text-slate-800"><b className="block text-xs text-emerald-800">معاينة توضيحية — بيانات تجريبية</b><p className="mt-2 whitespace-pre-wrap">{String(body).replace(/\{\{([a-zA-Z0-9_]+)\}\}/g, (original, variable) => preview[`body:${variable}`] ?? original)}</p></div>
