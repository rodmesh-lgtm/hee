...
export function CampaignWizard({ connections, templates, segments, eligibleContacts, sampleContacts = [] }: { sampleContacts?: PreviewContact[]; connections: ConnectionOption[]; templates: TemplateOption[]; segments: SegmentOption[]; eligibleContacts: number }) {
  ...
  return <form aria-label="إنشاء حملة واتساب" action={createWhatsAppCampaignAction} onSubmit={(event) => { if (step < steps.length - 1) { event.preventDefault(); moveNext(); } }} className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_60px_rgba(7,24,27,.06)]">
