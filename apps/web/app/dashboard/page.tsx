...
export default async function DashboardHomePage() {
  ...
  return <div className="space-y-4 pb-4">
    <section className="rounded-[26px] border border-[#e7e5f2] bg-[linear-gradient(135deg,#ffffff_0%,#faf8ff_65%,#f2edff_100%)] p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${effectivelyPublished ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{effectivelyPublished ? "منشورة" : "غير منشورة"}</span>{business.isVerified ? <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-black text-blue-700"><BadgeCheck className="h-3.5 w-3.5" />موثق</span> : null}</div><h1 className="mt-3 text-2xl font-black text-[#1f2552]">{business.name}</h1><p className="mt-1 text-sm text-slate-500">أدر هويتك الرقمية من مكان واحد.</p></div><div className="flex gap-2"><Link href="/dashboard/my-page" className="inline-flex h-10 items-center rounded-xl bg-[#6f3bd2] px-4 text-xs font-black text-white">تعديل الصفحة</Link><a href={effectivelyPublished ? publicUrl : "/preview"} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-[#ddd8f4] bg-white px-3 text-xs font-black text-[#5d49cc]"><Eye className="h-3.5 w-3.5" />{effectivelyPublished ? "فتح" : "معاينة"}</a></div></div>
    </section>
