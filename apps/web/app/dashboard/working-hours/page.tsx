  ...
  return <div className="space-y-4 pb-24 lg:pb-4">
    <section className="overflow-hidden rounded-[28px] border border-[#153438] bg-[#07181b] text-white shadow-[0_22px_70px_-46px_rgba(7,24,27,.78)]">
      <div className="grid lg:grid-cols-[1fr_340px]">
        <div className="relative p-5 sm:p-7"><div className="absolute -left-16 -top-20 h-52 w-52 rounded-full bg-[#00d8c6]/12 blur-3xl"/><div className="relative"><span className="inline-flex items-center gap-1.5 text-[8px] font-black tracking-[.15em] text-[#66e7d5]" dir="ltr"><Sparkles className="h-3.5 w-3.5"/>INFRO APPOINTMENTS</span><h1 className="mt-3 max-w-2xl text-[25px] font-black leading-tight tracking-tight sm:text-[30px]">جدول المواعيد والتوفر</h1><p className="mt-2 max-w-xl text-[11px] leading-6 text-slate-400 sm:text-xs">حدد الأيام والأوقات المتاحة أو المغلقة. سيعرض INFRO للزائر المواعيد الصحيحة ويمنع الحجز خارجها أو عند تعارض موعد قائم.</p></div></div>
    ...
    {saved?<div role="status" className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-bold text-emerald-700"><CheckCircle2 className="h-4 w-4"/>تم حفظ جدول المواعيد.</div>:null}
    ...
    <form action={updateWorkingHoursAction} className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_18px_60px_-48px_rgba(7,24,27,.5)]">
      <div className="flex flex-col gap-3 border-b border-slate-100 bg-[#fbfdfd] px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e9fbf8] text-[#008f87]"><CalendarDays className="h-4 w-4"/></span><div><span className="text-[8px] font-black tracking-[.14em] text-[#008f87]" dir="ltr">WEEKLY AVAILABILITY</span><h2 className="mt-1 text-sm font-black text-slate-950">توفر المواعيد الأسبوعي</h2></div></div><p className="max-w-md text-[9px] leading-5 text-slate-400">الفترة الثانية اختيارية للدوام المنقسم. لا تستخدمها إلا عند وجود توقف فعلي بين فترتين.</p></div>
