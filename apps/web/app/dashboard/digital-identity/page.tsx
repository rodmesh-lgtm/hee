...
export default async function DigitalIdentityPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="grid gap-4 lg:grid-cols-2"><article className="rounded-[24px] border border-[#e7e9f4] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><Link2 className="h-5 w-5 text-[#6f3bd2]" /><h2 className="font-black text-[#20264f]">بطاقة جهة الاتصال vCard</h2></div><p className="mt-2 text-xs leading-6 text-slate-500">ملف اتصال قابل للحفظ على الجوال باسم المنشأة، ويستخدم بيانات الهاتف والبريد والموقع المسجلة في HEE.</p><Link href="/api/dashboard/vcard" className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#6f3bd2] px-4 text-xs font-black text-white"><Download className="h-4 w-4" />تنزيل vCard</Link></article><article className="rounded-[24px] border border-[#e7e9f4] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><SearchCheck className="h-5 w-5 text-[#6f3bd2]" /><h2 className="font-black text-[#20264f]">اكتمال الهوية الرقمية</h2></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{identityChecks.map((item) => <div key={item.label} className={`rounded-xl border px-3 py-2 text-xs font-bold ${item.ok ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>{item.ok ? "✓" : "○"} {item.label}</div>)}</div><p className="mt-3 text-[11px] leading-5 text-slate-500">تتضمن الصفحة العامة بيانات SEO وOpen Graph وSchema.org، وتستخدم الحقول أعلاه مباشرة عند النشر.</p></article></section>
  </div>;
}
