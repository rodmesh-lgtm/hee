...
import { BadgeCheck, Download, ExternalLink, FileText, Link2, SearchCheck } from "lucide-react";
import { getCurrentUser } from "../../lib/auth";
import { getActiveBusinessForUser } from "../../lib/active-business";
import { updateCompanyProfileAction, removeCompanyProfileAction } from "../../actions/digital-identity";
import { DigitalPresenceForm } from "../../../components/dashboard/digital-presence-form";
import { IdentityAssets } from "../../../components/dashboard/identity-assets";
...
export default async function DigitalIdentityPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const presence = Array.isArray(params?.presence) ? params?.presence[0] : params?.presence;
  const publicUrl = `https://hee.sa/${business.slug}`;
  const socialReady = Boolean(business.instagramUrl || business.xUrl || business.tiktokUrl || business.snapchatUrl || business.facebookUrl);
  const identityChecks = [
    ...
    { label: "وسيلة تواصل", ok: Boolean(business.whatsapp || business.phone || business.email || business.website) },
    { label: "موقع المنشأة", ok: Boolean(business.city || business.address || business.googleMapsLink) },
    { label: "موقع إلكتروني", ok: Boolean(business.website) },
    { label: "شبكة اجتماعية", ok: socialReady },
    { label: "تهيئة SEO", ok: Boolean(business.metaTitle || business.metaDescription) },
  ...
  return <div className="space-y-4 pb-4">
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      ...
      <article className="rounded-[24px] border border-[#e7e9f4] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><Link2 className="h-5 w-5 text-[#6f3bd2]" /><h2 className="font-black text-[#20264f]">الرابط والاتصال</h2></div><p className="mt-2 text-xs leading-6 text-slate-500">استخدم رابط HEE الدائم في التوقيع والكروت والملفات، ونزّل vCard لإضافة بيانات المنشأة إلى جهات الاتصال.</p><div className="mt-4 rounded-2xl bg-[#faf9fd] p-4"><code dir="ltr" className="block max-w-full overflow-hidden text-ellipsis whitespace-nowrap text-[11px] text-slate-500">{publicUrl}</code><div className="mt-3 flex flex-wrap gap-2"><Link href={`/${business.slug}`} target="_blank" className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-[#6f3bd2] px-3 text-xs font-black text-white"><ExternalLink className="h-4 w-4" />فتح الصفحة</Link><Link href="/api/dashboard/vcard" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-[#ddd8f4] bg-white px-3 text-xs font-black text-[#5d49cc]"><Download className="h-4 w-4" />تنزيل vCard</Link></div></div><p className="mt-3 text-[11px] leading-5 text-slate-500">أزلنا مولّد QR الخارجي من مركز الهوية حتى لا تُرسل روابط صفحات العملاء إلى خدمة طرف ثالث أو تعتمد أصول الهوية على توفرها.</p></article>
    </section>

    <DigitalPresenceForm business={business} presenceStatus={presence} />

    <IdentityAssets businessName={business.name} logoUrl={business.logoUrl} primaryColor={business.primaryColor} phone={business.phone} whatsapp={business.whatsapp} email={business.email} website={business.website} publicUrl={publicUrl} />

    <section className="rounded-[24px] border border-[#e7e9f4] bg-white p-4 sm:p-5"><div className="flex items-center gap-2"><SearchCheck className="h-5 w-5 text-[#6f3bd2]" /><h2 className="font-black text-[#20264f]">اكتمال الهوية الرقمية</h2></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{identityChecks.map((item) => <div key={item.label} className={`rounded-xl border px-3 py-2 text-xs font-bold ${item.ok ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}>{item.ok ? "✓" : "○"} {item.label}</div>)}</div><p className="mt-3 text-[11px] leading-5 text-slate-500">تتضمن الصفحة العامة بيانات SEO وOpen Graph وSchema.org، وتستخدم الحقول أعلاه مباشرة عند النشر.</p></section>
  </div>;
}
