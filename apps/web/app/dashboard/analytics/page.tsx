...
export default async function DashboardAnalyticsPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4 pb-4">
    ...
    {!advancedAnalytics ? <div className="rounded-2xl border border-[#ddd8f4] bg-[#f8f6ff] p-4 text-sm font-bold text-[#5d49cc]">الباقة المجانية تعرض آخر 7 أيام. الترقية تفتح تحليلات 30 و90 يومًا.</div> : null}{!effectivelyPublished ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold text-amber-800">{business.isPublished && !user.emailVerifiedAt ? "أكد بريد حسابك لإتاحة الصفحة وبدء تسجيل زيارات العملاء." : "انشر صفحتك أولاً لبدء تسجيل زيارات العملاء."}</div> : null}
