...
test("campaign launch readiness fails closed when web release identity cannot be proven", () => {
  const result = readiness({ currentTime: new Date("2026-08-31T10:00:00Z"), expectedReleaseSha: null, heartbeat: { lastSucceededAt: new Date("2026-08-31T09:59:00Z"), lastErrorCode: null, releaseSha } });
...
test("campaign launch readiness rejects a healthy worker from a different release", () => {
  const result = readiness({ currentTime: new Date("2026-08-31T10:00:00Z"), expectedReleaseSha: releaseSha, heartbeat: { lastSucceededAt: new Date("2026-08-31T09:59:00Z"), lastErrorCode: null, releaseSha: "b".repeat(40) } });
...
test("campaign launch readiness fails closed when worker has never succeeded", () => {
  const result = readiness({ currentTime: new Date("2026-08-31T10:00:00Z"), expectedReleaseSha: releaseSha, heartbeat: { lastSucceededAt: null, lastErrorCode: null, releaseSha } });
...
test("campaign launch readiness fails closed on the latest worker error", () => {
  const result = readiness({ currentTime: new Date("2026-08-31T10:00:00Z"), expectedReleaseSha: releaseSha, heartbeat: { lastSucceededAt: new Date("2026-08-31T09:59:00Z"), lastErrorCode: "WHATSAPP_DELIVERIES_FAILED", releaseSha } });
...
test("campaign launch readiness rejects stale or implausibly future heartbeats", () => {
  const currentTime = new Date("2026-08-31T10:00:00Z");
  const stale = readiness({ currentTime, expectedReleaseSha: releaseSha, heartbeat: { lastSucceededAt: new Date(currentTime.getTime() - WHATSAPP_OPERATIONS_HEARTBEAT_MAX_AGE_MS), lastErrorCode: null, releaseSha } });
  const future = readiness({ currentTime, expectedReleaseSha: releaseSha, heartbeat: { lastSucceededAt: new Date(currentTime.getTime() + 61_000), lastErrorCode: null, releaseSha } });
...
test("campaign launch readiness fails closed when database time cannot be proven", () => {
  const result = readiness({ currentTime: null, expectedReleaseSha: releaseSha, heartbeat: { lastSucceededAt: new Date(), lastErrorCode: null, releaseSha } });
...
test("campaign launch action proves worker readiness before enqueuing and UI disables unsafe launch", () => {
  ...
  assert.match(page, /الإرسال متوقف مؤقتًا للحماية/);
  assert.match(page, /خدمة الإرسال تحتاج إلى التحديث لتطابق إصدار المنصة/);
});
