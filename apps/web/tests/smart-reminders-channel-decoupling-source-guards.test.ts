...
test("non-WhatsApp reminders store nullable Meta bindings while WhatsApp remains database-enforced", () => {
  ...
  assert.match(migration, /SmartReminder_whatsapp_binding_required/);
  assert.match(migration, /SmartReminderDelivery_whatsapp_binding_required/);
  assert.match(migration, /'whatsapp' = ANY\("deliveryChannels"\)/);
});
...
test("scheduler only requires Meta bindings for the WhatsApp channel", () => {
  assert.match(scheduler, /channel === "whatsapp" && \(!reminder\.connectionId \|\| !reminder\.templateId\)/);
  assert.match(scheduler, /channel === "whatsapp" \? reminder\.connectionId : null/);
  assert.match(scheduler, /channel === "whatsapp" \? reminder\.templateId : null/);
});

test("email and in-app delivery load context before any WhatsApp binding requirement", () => {
  ...
  const bindingAt = worker.indexOf('REMINDER_WHATSAPP_BINDING_MISSING');
  assert.ok(emailAt > 0 && inAppAt > emailAt && bindingAt > inAppAt);
});

test("the create form keeps non-WhatsApp channels available when Meta is unavailable", () => {
  ...
  assert.match(form, /لا تحتاج إلى ربط واتساب لاستخدام البريد أو إشعارات INFRO/);
});
