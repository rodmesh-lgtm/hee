...
test("production deploy stages a production artifact before any canonical promotion", () => {
  ...
  const stageSmoke = workflow.indexOf("Verify staged Production deployment before promotion");
  const promote = workflow.indexOf("Promote verified staged deployment to Production domains");
  const canonical = workflow.indexOf("Verify canonical domain serves exact release SHA after promotion");
  ...
  assert.ok(stageSmoke > stage, "staged deployment must be smoke-tested before promotion");
  assert.ok(promote > stageSmoke, "promotion must happen only after staged smoke succeeds");
  assert.ok(canonical > promote, "canonical release must be checked after promotion");
  assert.ok(canonicalSmoke > canonical, "customer surfaces must be checked after canonical release convergence");
  ...
  assert.match(workflow, /test "\$release_env" = "production"/);
