...
test("production deploy stages and proves exact-SHA core-ready Production before any canonical promotion", () => {
  ...
  assert.match(workflow, /staged_request \/api\/release/);
  assert.match(workflow, /staged_request \/api\/maintenance\/status/);
  assert.match(workflow, /staged_request \/api\/health\/web-ready/);
  assert.match(workflow, /staged_request "\$path"/);
  assert.match(workflow, /vercel@\$\{VERCEL_CLI_VERSION\}" curl/);
  assert.match(workflow, /--deployment "\$deployment_url" --scope "\$VERCEL_ORG_ID" --yes/);
  assert.match(workflow, /Invalid staged Vercel deployment URL/);
  assert.match(workflow, /Staged deployment identity or readiness mismatch before authenticated probe/);
  const stagedChecks = workflow.slice(stageSmoke, promote);
  assert.doesNotMatch(stagedChecks, /--location|--verbose|--debug/);
  assert.doesNotMatch(workflow, /protection disable|ssoProtection|passwordProtection/);
