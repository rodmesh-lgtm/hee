...
test("production deploy stages and proves exact-SHA non-maintenance Production before any canonical promotion", () => {
  ...
  const stageSmoke = workflow.indexOf("Verify staged exact-SHA non-maintenance Production before promotion");
  const promote = workflow.indexOf("Promote verified staged deployment to Production domains");
  const canonical = workflow.indexOf("Verify canonical domain serves exact-SHA non-maintenance Production after promotion");
  ...
  assert.ok(stageSmoke > stage, "staged exact-SHA non-maintenance deployment must be proven before promotion");
  assert.ok(promote > stageSmoke, "promotion must happen only after staged proof succeeds");
  assert.ok(canonical > promote, "canonical exact-SHA non-maintenance state must be checked after promotion");
