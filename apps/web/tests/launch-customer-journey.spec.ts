  ...
  test("registers, verifies email, onboards, uses mobile dashboard, publishes, shares, and signs back in", async ({ browser }) => {
    ...
    try {
      ...
      await test.step("logout protects dashboard and password login restores access", async () => {
        await page.goto(`${baseUrl}/dashboard`, { waitUntil: "domcontentloaded" }); await page.locator("#hee-dashboard-mobile-more-button").click(); const logoutButton = page.getByRole("dialog", { name: "قائمة لوحة التحكم" }).getByRole("button", { name: "تسجيل الخروج" }); const loggedOut = page.waitForURL((url) => url.origin === new URL(baseUrl).origin && url.pathname === "/", { timeout: 20_000 }); await logoutButton.click({ noWaitAfter: true }); await loggedOut;
