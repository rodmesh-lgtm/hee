...
async function waitForMainFrameQuiescence(page: import("@playwright/test").Page) {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const navigation = await page.waitForEvent("framenavigated", { predicate: (frame) => frame === page.mainFrame(), timeout: 1_500 }).catch(() => null);
...
test.describe.serial("launch customer journey", () => {
  test("registers, verifies email, onboards, uses mobile dashboard, publishes, shares, and signs back in", async ({ browser }) => {
    ...
    // Playwright retries recreate the test worker after a timeout. Keep the DB lifecycle
    // inside the test so a retry never inherits a Prisma adapter backed by an ended pool.
    ...
    try {
      ...
      await test.step("customer adds first service and publishes the verified page", async () => {
        await page.goto(`${baseUrl}/dashboard/services`, { waitUntil: "domcontentloaded" }); const addForm = page.getByRole("form", { name: "إضافة خدمة" }); await addForm.getByLabel("اسم الخدمة").fill("خدمة رحلة الإطلاق"); await addForm.getByRole("button", { name: "إضافة" }).click();
        ...
        await page.goto(`${baseUrl}/dashboard/my-page`, { waitUntil: "domcontentloaded" }); const publishResponse = page.waitForResponse((response) => { try { const url = new URL(response.url()); return response.request().method() === "POST" && url.origin === new URL(baseUrl).origin && url.pathname === "/dashboard/my-page"; } catch { return false; } }, { timeout: 20_000 }); await page.getByRole("button", { name: "نشر الصفحة" }).click(); const response = await publishResponse; expect(response.ok()).toBe(true); await expect(page.getByText("منشورة", { exact: true })).toBeVisible({ timeout: 20_000 });
        const user = await db.user.findUniqueOrThrow({ where: { email }, select: { id: true } }); await expect.poll(async () => (await db.business.findFirst({ where: { ownerId: user.id }, select: { isPublished: true } }))?.isPublished, { timeout: 20_000 }).toBe(true); await waitForMainFrameQuiescence(page);
        await page.goto(`${baseUrl}/${slug}`, { waitUntil: "domcontentloaded" }); await expect(page.getByRole("heading", { name: "منشأة رحلة الإطلاق" })).toBeVisible(); await expect(page.getByRole("heading", { name: "أبرز ما نقدمه" })).toBeVisible(); await expect(page.getByText("خدمة رحلة الإطلاق")).toBeVisible();
      });
      await test.step("public share fallback gives clear mobile feedback", async () => { await page.evaluate(() => { Object.defineProperty(navigator, "share", { configurable: true, value: undefined }); Object.defineProperty(navigator, "clipboard", { configurable: true, value: { writeText: async () => undefined } }); }); const shareButton = page.getByRole("button", { name: "مشاركة الصفحة" }); await shareButton.click(); await expect(page.getByText("تم نسخ الرابط")).toBeVisible(); });
      ...
      await test.step("logout protects dashboard and password login restores access", async () => {
        await page.goto(`${baseUrl}/dashboard`, { waitUntil: "domcontentloaded" }); await page.locator("#infro-dashboard-mobile-more").click(); const logoutButton = page.getByRole("dialog", { name: "قائمة لوحة التحكم" }).getByRole("button", { name: "تسجيل الخروج" }); const loggedOut = page.waitForURL((url) => url.origin === new URL(baseUrl).origin && url.pathname === "/", { timeout: 20_000 }); await logoutButton.click({ noWaitAfter: true }); await loggedOut;
        await page.goto(`${baseUrl}/dashboard`, { waitUntil: "domcontentloaded" }); await page.waitForURL("**/login", { timeout: 20_000 }); await page.getByLabel("البريد الإلكتروني").fill(email); await page.locator('input[name="password"]').fill(password); const signedIn = page.waitForURL((url) => url.origin === new URL(baseUrl).origin && url.pathname === "/dashboard", { timeout: 20_000 }); await page.getByRole("button", { name: "تسجيل الدخول" }).click({ noWaitAfter: true }); await signedIn;
      });
      expect(consoleErrors).toEqual([]);
    } finally {
      await page.close().catch(() => undefined);
      await cleanupByEmail().catch(() => undefined);
