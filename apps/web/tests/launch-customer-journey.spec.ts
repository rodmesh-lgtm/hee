  ...
  test("registers, verifies email, onboards, uses mobile dashboard, publishes, shares, and signs back in", async ({ browser }) => {
    ...
    try {
      ...
      await test.step("customer adds first service and publishes the verified page", async () => {
        ...
        await page.goto(`${baseUrl}/${slug}`, { waitUntil: "domcontentloaded" }); await expect(page.getByRole("heading", { name: "منشأة رحلة الإطلاق" })).toBeVisible(); await page.getByRole("button", { name: /خدماتنا/ }).click(); await expect(page.getByText("خدمة رحلة الإطلاق")).toBeVisible();
      });
