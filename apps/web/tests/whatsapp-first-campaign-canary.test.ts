...
test("delivery queue enforces canary below every immediate or scheduled enqueue caller", () => {
  ...
  assert.match(page, /خمسة مستلمين/);
  assert.match(page, /إكمال الإطلاق/);
  assert.match(page, /delivered > 0/);
});
