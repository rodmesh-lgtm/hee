...
async function main() {
  ...
  try {
    ...
    await mustReject(
      client,
      `INSERT INTO "BillingPayment" ("id","businessId","planId","subscriptionId","providerPaymentId","providerGivenId","kind","amount","currency","status","attempt","paidAt","createdAt","updatedAt") VALUES ('billing-audit-paid-no-receipt-${suffix}','${businessA}','${planId}','${subscriptionA}','${randomUUID()}','${randomUUID()}','initial',19900,'SAR','paid',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`,
      "23514",
      "paid payment without immutable receipt snapshot",
    );
    await mustReject(
      client,
      `INSERT INTO "BillingPayment" ("id","businessId","planId","providerGivenId","kind","amount","currency","status","attempt","receiptSellerLegalName","createdAt","updatedAt") VALUES ('billing-audit-partial-receipt-${suffix}','${businessA}','${planId}','${randomUUID()}','initial',19900,'SAR','created',1,'Partial Seller',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)`,
      "23514",
      "partial receipt snapshot",
    );
    ...
    const constraints = await client.query<{ conname: string }>(`SELECT conname FROM pg_constraint WHERE conname IN ('Subscription_payment_method_business_fkey','BillingPayment_subscription_business_fkey','BillingPayment_amount_positive','BillingPayment_currency_sar','BillingPayment_receipt_snapshot_complete','BillingPayment_paid_state_complete')`);
    assert.deepEqual(new Set(constraints.rows.map((row) => row.conname)), new Set([
      "Subscription_payment_method_business_fkey",
      "BillingPayment_subscription_business_fkey",
      "BillingPayment_amount_positive",
      "BillingPayment_currency_sar",
